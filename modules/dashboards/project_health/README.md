...
>
> The `library.cognite` package includes Deployment Packs developed by the Value Delivery Accelerator team (RMDM, Atlas AI, Context Quality Dashboard, Project Health Dashboard, etc.). A `checksum` field is not required for current Toolkit versions.
