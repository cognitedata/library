"""Config path registry and access-control mirroring."""
...
from governance_build.config_sources import (
    PRIMARY_CONFIG,
    merge_access_control_into_document,
...
def test_merge_access_control_preserves_other_keys():
    base = {"toolkit": {"x": 1}, "dimensions": {"a": 1}, "extra": 2}
    src = {
        "dimensions": {"b": 2},
        "groups": {"global": {"source_ids": {"from_src": "x"}}},
    }
    out = merge_access_control_into_document(base, src)
...
def test_mirror_writes_multiple_files(tmp_path):
    ...
    written, skipped = mirror_access_control_slice(tmp_path, slice_doc, dry_run=False)
