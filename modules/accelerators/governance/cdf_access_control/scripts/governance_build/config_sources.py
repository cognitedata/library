    ...
    if primary.is_file():
        try:
            ...
            if isinstance(doc, dict):
                ac = doc.get("access_control_ui")
                if isinstance(ac, dict):
                    raw = ac.get("mirror_config_paths")
            ...
            pass
    extra = os.environ.get("CDF_ACCESS_CONTROL_MIRROR_CONFIGS", "")
...
def merge_access_control_into_document(
...
def mirror_access_control_slice(
    ...
    for rel in mirror_config_relpaths(module_root):
        ...
        merged = merge_access_control_into_document(base, slice_doc)
        if dry_run:
            written.append(rel)
            continue
