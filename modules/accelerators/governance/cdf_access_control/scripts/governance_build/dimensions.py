"""Load and validate ``dimensions`` registry (hierarchy + list)."""
...
def require_hierarchy(dimensions: Mapping[str, Any], name: str) -> Dict[str, Any]:
    block = dimensions.get(name)
    if not isinstance(block, dict):
        raise ValueError(f"dimensions.{name} must be a mapping")
    if block.get("type") != HIERARCHY:
        raise ValueError(f"dimensions.{name} must have type: hierarchy")
    return block
