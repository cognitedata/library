...
export function DimensionsEditor({ doc, onChange }: Props) {
  ...
  const [newType, setNewType] = useState<"hierarchy" | "list" | "other">("hierarchy");
  ...
  const createDimension = () => {
    ...
    if (newType === "hierarchy") {
      block = { order: keys.length, type: "hierarchy", levels: ["site"], locations: [] };
    } else if (newType === "list") {
      block = { order: keys.length, type: "list", items: [{ id: "item_1", name: "" }] };
    } else {
      try {
...
function ListItemRow({
  ...
  return (
    ...
    >
      ...
      <td>
        <button type="button" className="gov-btn gov-btn--sm gov-btn--danger" onClick={onRemove}>
          ×
        </button>
      </td>
    </tr>
