...
export function ArtifactTree({ paths, selectedPath, onSelectFile }: Props) {
  ...
  if (paths.length === 0) {
    treeBody = <p className="gov-hint gov-art-tree-empty">—</p>;
  } else if (noMatches) {
    treeBody = <p className="gov-hint gov-art-tree-empty">{t("artifacts.noSearchResults")}</p>;
  } else {
    treeBody = (
      <ul className="gov-art-tree" role="tree">
        {tree.map((node, i) => (
          <ArtifactTreeRows
  ...
  return (
    <div className="gov-art-tree-panel">
      <TreeContextMenuPortal menu={ctxMenu.menu} onClose={ctxMenu.close} classPrefix="gov" />
      <label className="gov-label gov-art-tree-search-label" title={t("artifacts.searchLabel.tooltip")}>
        {t("artifacts.searchLabel")}
        <input
          type="search"
          className="gov-input gov-art-tree-search"
          value={query}
      ...
      <div
        className="gov-artifact-tree-wrap"
        onContextMenu={(e) => {
          const tEl = e.target as HTMLElement;
          if (tEl.closest(".gov-art-tree-li")) return;
...
function ArtifactTreeRows({
  ...
  if (node.kind === "file") {
    ...
    return (
      <li className="gov-art-tree-li" role="none">
        <button
          type="button"
          role="treeitem"
          className={active ? "gov-art-tree-file gov-art-tree-file--active" : "gov-art-tree-file"}
  ...
  return (
    <li className="gov-art-tree-li" role="none">
      <div
        className="gov-art-tree-dir-row"
      ...
      >
        <button
          type="button"
          className="gov-art-tree-chevron"
        ...
        <button
          type="button"
          role="treeitem"
          className="gov-art-tree-dir-label"
          onClick={() => onToggleDir(node.relPath)}
        >
          {node.name}
        </button>
      ...
      {isOpen && (
        <ul className="gov-art-tree-nested" role="group">
