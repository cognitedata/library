...
class MatchTracker:
    ...
    def add_match(self, asset_ext_id: str, entity_ext_id: str, match_data: Dict[str, Any]) -> bool:
...
class OptimizedRuleMapper:
    ...
    def __init__(self, rule_mappings: List[Dict[str, Any]], logger: Optional[CogniteFunctionLogger] = None):
        ...
        for rule in rule_mappings:
            ...
            if "entity_regex" in rule:
                ...
                try:
                    self.compiled_patterns[pattern_key] = re.compile(rule["entity_regex"])
            ...
            if "asset_regex" in rule:
                ...
                try:
                    self.compiled_patterns[pattern_key] = re.compile(rule["asset_regex"])
                    self.rule_data[pattern_key] = rule
                except re.error as e:
                    # Log but don't fail
                    if self.logger:
...
class BatchProcessor:
    ...
    def process_entities_in_batches(self, entities: List[Dict[str, Any]], 
                                   process_func, logger: CogniteFunctionLogger, 
...
class ConcurrentDataLoader:
    ...
    def load_data_concurrently(self, load_functions: List[Tuple[Callable, tuple]], 
                              logger: CogniteFunctionLogger) -> List[Any]:
        """Load data concurrently"""
        
        with time_operation("Concurrent data loading", logger):
            with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                # Submit all tasks
                futures = [executor.submit(func, *args) for func, args in load_functions]
...
class OptimizedMatchingEngine:
    ...
    def apply_rule_mappings_optimized(self, assets: List[Dict[str, Any]], 
                                     entities: List[Dict[str, Any]], 
                                     rule_mapper: OptimizedRuleMapper) -> List[Dict[str, Any]]:
        ...
        with time_operation("Optimized rule mapping", self.logger):
            ...
            for entity in entities:
                entity_rule_keys = rule_mapper.get_rule_keys(entity.get("name", ""), "entity")
                        ...
                        for matching_asset in asset_index[rule_key]:
                            if self.match_tracker.add_match(
                                ...
                                entity.get("entity_ext_id", ""),
                                {
                                    ...
                                    "asset_ext_id": matching_asset.get("asset_ext_id", ""),
                                }
                            ):
                                matches_found += 1
    ...
    def batch_update_entities(self, updates: List[Any], batch_size: int = 2000):
        ...
        for i in range(0, len(updates), batch_size):
            ...
            try:
                ...
                self.logger.info(f"Updated batch {i//batch_size + 1}: {len(batch)} entities")
            except Exception as e:
                self.logger.warning(f"Large batch failed, retrying with smaller chunks: {e}")
                # Split batch and retry smaller chunks
                for j in range(0, len(batch), batch_size // 4):
                    small_batch = batch[j:j + batch_size // 4]
                    self.robust_api_call(self.client.data_modeling.instances.apply, small_batch)
...
class PerformanceBenchmark:
    ...
    def log_summary(self):
        ...
        for name, stats in summary.items():
            self.logger.info(f"  {name}: {stats['count']} calls, avg {stats['average']:.2f}s, total {stats['total']:.2f}s")
