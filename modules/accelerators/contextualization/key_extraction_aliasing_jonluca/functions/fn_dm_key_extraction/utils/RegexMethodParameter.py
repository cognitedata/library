...
class RegexMethodParameter(BaseModel):
    ...
    validation_pattern: Optional[str] = Field(
        None,
        description="Additional pattern for post-extraction validation (optional, e.g., '^P\\d{2,4}[A-Z]?$').",
    )
    ...
    reassemble_format: Optional[str] = Field(
        None,
        description="Template for reassembling captured components (optional, e.g., '{prefix}-{number}{suffix}').",
    )
