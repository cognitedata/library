...
class PositionalDetection(BaseModel):
    ...
    pattern: str = Field(
        ..., description="A regular expression that captures the token"
    )
    confidence_boost: float = Field(
        0.0, description="Boost to apply to the confidence score if matched"
    )
...
class FrequencyAnalysis(BaseModel):
    ...
    analyze_corpus: bool = Field(
        False,
        description="Whether to analyze the entire corpus for frequency. If set to true, will analyze all fields present in the ingested entity.",
    )
    min_frequency: int = Field(
        1,
        description="Minimum frequency count for a term to be considered significant.",
    )
    pattern_stability_threshold: float = Field(
        ...,
        description="Minimum stability threshold for a pattern to be considered reliable.",
    )
    common_prefix_detection: bool = Field(
        True, description="Whether to enable common prefix detection."
    )
    common_suffix_detection: bool = Field(
        True, description="Whether to enable common suffix detection."
    )


class EquipmentTypeCorrelationParameters(BaseModel):
    enabled: bool = Field(
        False, description="Whether to enable equipment type correlation."
    )
    type_indicators: dict[str, List[str]] = Field(
        default_factory=dict,
        description="Indicators for different equipment types (i.e. 'pump': ['centrifugal', 'positive displacement', etc...]).",
    )
...
class ContextInference(BaseModel):
    ...
    surrounding_keywords: dict[str, List[str]] = Field(
        default_factory=lambda: {"positive": [], "negative": []},
        description="Keywords indicating positive or negative context.",
    )
    context_window: int = Field(
        5,
        description="Number of tokens to consider as context (i.e. characters before/after).",
    )
    keyword_proximity_bonus: float = Field(
        0.0, description="Bonus to apply for keywords found within the context window."
    )
    equipment_type_correlation: EquipmentTypeCorrelationParameters = Field(
        None, description="Parameters for equipment type correlation."
    )
...
class ExampleBasedLearning(BaseModel):
    ...
    learning_mode: Literal["similarity"] = Field(
        "similarity",
        description="The learning mode to be used for example-based learning.",
    )
    ...
    similarity_threshold: float = Field(
        ..., description="The similarity threshold for matching examples."
    )
    feature_extraction: dict[str, Union[bool, List[int]]] = Field(
        ..., description="Feature extraction parameters for example-based learning."
    )


HeuristicConfigParameter = Union[
    PositionalDetection, FrequencyAnalysis, ContextInference
]
...
class HeuristicStrategyConfig(BaseModel):
    ...
    strategy_id: str = Field(
        ..., description="Identifier for the specific strategy being applied."
    )
    weight: float = Field(
        ..., description="Weight of this strategy in the final score (e.g., 0.3)."
    )
    method: Literal[
    ...
    ] = Field(..., description="The specific method being used.")
...
class ScoringParameters(BaseModel):
    ...
    normalize_scores: bool = Field(
        False, description="Whether to normalize scores before aggregation."
    )
