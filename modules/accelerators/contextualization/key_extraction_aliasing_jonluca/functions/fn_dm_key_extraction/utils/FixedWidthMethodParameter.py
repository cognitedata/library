...
class FieldDefinition(BaseModel):
    ...
    line: int = Field(
        None,
        description="The 0-indexed line # of a multi-line record parse. Will not use multi-line parsing if this field is not defined for one or more field definitions.",
    )


# Use Literal for the specific string options to enhance type checking
EncodingOption = Literal[
    "utf-8", "latin-1", "ascii"
]  # Added 'ascii' as a common option
...
class FixedWidthMethodParameter(BaseModel):
    ...
    skip_lines: int = Field(
        0,
        description="Number of initial lines to skip (e.g., 2). This will not change the index of multi-line parsing. This will skip the first x records, however they are defined. This does not assume that lines are split by new line chars.",
    )
