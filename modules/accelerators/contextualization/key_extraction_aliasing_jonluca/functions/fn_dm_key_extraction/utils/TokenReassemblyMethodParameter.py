...
class TokenPattern(BaseModel):
    ...
    position: Optional[int] = Field(None, description="Position of the token in the input string.")

    required: bool = Field(False, description="Whether the token is required.")

    component_type: str = Field("unknown", description="Type of the token component.")
...
class TokenizationParameters(BaseModel):
    ...
    token_patterns: List[TokenPattern] = Field(
        ..., description="Token extraction patterns."
    )

    separator_pattern: Union[str, list] = Field(
        ...,
    ...
    extract_from_multiple_fields: Optional[List[str]] = Field(
        None,
        description="Cross-field token extraction (e.g., ['name', 'description']).",
...
class AssemblyRule(BaseModel):
    ...
    format: str = Field(
        ..., description="Template with placeholders (e.g., '{site}-{unit}-{tag}')."
    )
    ...
    conditions: Optional[Dict[str, Any]] = Field(
        None,
        description="Conditions for applying this rule (e.g., {'token_count': 3}).",
    )
...
class ValidationParameters(BaseModel):
    ...
    min_tokens: Optional[int] = Field(
        None, description="Minimum tokens required (e.g., 3)."
    )
    max_tokens: Optional[int] = Field(
        None, description="Maximum tokens allowed (e.g., 5)."
    )
    validate_assembled: bool = Field(
        False, description="Validate final assembled result (e.g., True)."
    )
    validation_pattern: Optional[str] = Field(
        None,
        description="Regex for assembled result validation (e.g., '^[A-Z]+-\\d+-[A-Z]+\\d+$').",
    )
...
class TokenReassemblyMethodParameter(BaseModel):
    ...
    def __init__(self, **data):
        ...
        self.tokenization.token_patterns = sorted(
            self.tokenization.token_patterns, key=lambda x: x.position
        )
    ...
    # removed for now, broke too many things
    ...
    # def validate_and_cleanup_tokens(self):
        ...
        # filtered_patterns = [
        #     pattern
        #     for pattern in self.tokenization.token_patterns
