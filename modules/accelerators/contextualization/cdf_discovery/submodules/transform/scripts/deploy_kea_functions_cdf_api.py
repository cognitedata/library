...
``common/``, and the function folder (``fn_dm_*``) so
...
def _functions_yaml_path(module_root: Path) -> Path:
    return module_root / "submodules/transform/functions" / "functions.Function.yaml"
...
def deploy_kea_functions(
    ...
    *,
    ...
    Args:
        ...
        module_root: Path to the ``cdf_discovery_aliasing`` module root.
    ...
    functions_root = module_root / "submodules/transform/functions"
