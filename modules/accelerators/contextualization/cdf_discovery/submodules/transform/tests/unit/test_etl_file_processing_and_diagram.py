...
def test_file_annotation_in_local_pipeline_specs():
    ...
    spec = etl_local_pipeline_specs().get("fn_discovery_etl_file_annotation")
    assert spec == ("fn_discovery_etl_file_annotation.pipeline", "file_annotation")
    launch_spec = etl_local_pipeline_specs().get("fn_discovery_etl_file_annotation_launch")
    finalize_spec = etl_local_pipeline_specs().get("fn_discovery_etl_file_annotation_finalize")
    barrier_spec = etl_local_pipeline_specs().get("fn_discovery_etl_file_annotation_barrier")
    assert launch_spec == ("fn_discovery_etl_file_annotation_launch.pipeline", "file_annotation_launch")
    assert finalize_spec == ("fn_discovery_etl_file_annotation_finalize.pipeline", "file_annotation_finalize")
    assert barrier_spec == ("fn_discovery_etl_file_annotation_barrier.pipeline", "file_annotation_barrier")
...
def test_file_pattern_extract_fanout_plan_compiles_to_fanout_function():
    ...
    assert fanout_plan["function_external_id"] == "fn_discovery_etl_fanout_plan"
...
def test_build_dynamic_detect_pack_tasks_one_per_pack():
    ...
    tasks = build_dynamic_detect_pack_tasks(
        ...
        child_function_external_id="fn_discovery_etl_file_annotation",
    )
    assert len(tasks) == 1
