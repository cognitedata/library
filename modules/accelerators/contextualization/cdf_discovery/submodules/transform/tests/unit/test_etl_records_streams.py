...
def test_etl_handle_query_records_in_memory() -> None:
    ...
    root = Path(__file__).resolve().parents[2] / "submodules/transform/functions"
    if str(root) not in sys.path:
        sys.path.insert(0, str(root))

    from fn_discovery_etl_records_query.handler import etl_handle_query_records
    ...
    summary = etl_handle_query_records("fn_discovery_etl_records_query", data, client, None)
