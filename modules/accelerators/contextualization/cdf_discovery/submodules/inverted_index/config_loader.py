...
def module_root() -> Path:
    """Inverted-index package root (``inverted_index/`` under cdf_discovery)."""
...
def load_yaml_config(path: Path | None = None) -> dict[str, Any]:
    cfg_path = path or (module_root() / "default.config.yaml")
...
def load_direct_relation_preset() -> dict[str, Any]:
    preset_path = module_root() / "config" / "direct_relation.cdm_preset.yaml"
    if not preset_path.is_file():
        return {"enabled": True, "views": {}, "links": {}}
    with preset_path.open(encoding="utf-8") as fh:
        data = yaml.safe_load(fh) or {}
    return data if isinstance(data, dict) else {}
