...
def validate_transform_config(cfg: Mapping[str, Any]) -> None:
    from cdf_fn_common.etl_pipeline_steps import parse_steps_list
    from cdf_fn_common.etl_transform.step_io import validate_transform_step_io

    nested = parse_steps_list(cfg)
    if nested:
        for index, step in enumerate(nested):
            if step.get("enabled") is False:
                continue
            ctx = f"transform config step[{index}]"
            validate_transform_step_io(step, context=ctx)
