...
class SubstitutionVariantsHandler(AbstractTransformHandler):
    ...
    description = (
        "Fan-out: when match_literal appears in the working string, emit one output row per entry in "
        "variants[] (must be unique). Use for alias expansion and multi-representation tags."
    )
    multi_value = True
