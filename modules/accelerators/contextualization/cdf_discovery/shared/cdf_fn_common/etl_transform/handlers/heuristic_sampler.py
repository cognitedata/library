...
class HeuristicSamplerHandler(AbstractTransformHandler):
    ...
    description = (
        "Find regex matches in the working string from literal samples (longest match wins) or a custom "
        "pattern. Control volume with max_results (1 default, 0/null = all, N = cap). on_no_match: "
        "keep_working, empty, or default + default_value."
    )
    multi_value = False
