...
class RegexSubstitutionHandler(AbstractTransformHandler):
    ...
    description = (
        "Apply one or more regex pattern → replacement rules to the working string built from "
        "fields[] and output_template. Supports multiple patterns, optional flags (I/M), and per-pattern "
        "replace counts. Use for tag extraction, prefix stripping, and normalization."
    )
