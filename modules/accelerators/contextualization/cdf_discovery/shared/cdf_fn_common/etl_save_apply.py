...
from cdf_fn_common.index_entry_bridge import (
    cohort_index_rows_to_index_entries,
...
def etl_persist_inverted_index_save(
    ...
    """Persist inverted-index cohort rows via contextualization storage adapters."""
    cfg = _prepare_save_cfg(data)
    run_id = require_pipeline_run_key(data)
    data["run_id"] = run_id
    task_id = _first_nonempty(data.get("task_id"), fn_external_id)
    ...
    if log and hasattr(log, "info"):
        log.info(
            "%s inverted_index_save rows_read=%s created=%s updated=%s backend=%s scope=%s dry_run=%s",
            fn_external_id,
            rows_read,
            entries_created,
            entries_updated,
    ...
    return {
        ...
        "task_id": task_id,
        "rows_read": rows_read,
        "rows_written": entries_created + entries_updated,
        "entries_created": entries_created,
        "entries_updated": entries_updated,
        "index_rows_written": entries_created + entries_updated,
        "run_id": run_id,
