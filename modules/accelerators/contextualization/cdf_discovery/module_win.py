...
Windows entrypoint — same CLI as ``module.py``, but resolves ``npm.cmd`` via ``shutil.which``
so ``subprocess`` can launch npm on Windows.

  python module_win.py ui [--api-host HOST] [--api-port PORT] [--vite-port PORT] [--no-browser]
  python module_win.py build [--config default.config.yaml] [--dry-run] [--force]
  python module_win.py build --clean [--yes]
  python module_win.py build --check-generated
  python module_win.py build --build-inverted-index-triggers [--force]
  python module_win.py build --check-inverted-index-triggers
  python module_win.py transform build [--pipeline ID | --template ID] [--scoped] [--dry-run]
  python module_win.py transform run [--instance ID | --template ID] [--dry-run] [--predecessor-mode in_memory|cohort]
  python module_win.py transform deploy-scope [--dry-run]
  python module_win.py index {build-metadata|query|target-driven|…}  (inverted index CLI)
    ...
    try:
        ...
        with _log_path.open("a", encoding="utf-8") as _lf:
            _lf.write(_json.dumps({
                "sessionId": "e2a6dd",
                "hypothesisId": "A-B",
                "location": "module_win.py:_run_ui:deps_check",
...
def _run_index(argv: List[str]) -> int:
    ...
    if argv and argv[0] == "ui":
        print(
            ...
            "Use: python module_win.py ui  (from cdf_discovery)",
            file=sys.stderr,
        )
        return 2
    ensure_sys_path()
...
def main() -> None:
    ...
    if args[0] == "transform":
        if len(args) < 2:
            print("Usage: module_win.py transform {build|run|deploy-scope} …", file=sys.stderr)
            raise SystemExit(2)
        sub = args[1]
        sub_argv = args[2:]
        if sub == "build":
