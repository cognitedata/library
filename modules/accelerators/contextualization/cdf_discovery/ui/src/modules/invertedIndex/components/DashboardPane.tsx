...
import { createInvertedIndexTab } from "../utils/indexTabs";
...
type Props = {
  refreshKey: number;
  onOpenTab?: (tab: InvertedIndexDocumentTab) => void;
};
...
export function DashboardPane({ refreshKey, onOpenTab }: Props) {
  ...
  const openQuick = (kind: "inverted_index_build_metadata" | "inverted_index_query" | "inverted_index_configuration", labelKey: Parameters<typeof t>[0]) => {
    if (!onOpenTab) return;
    const label = t(labelKey);
    onOpenTab(createInvertedIndexTab(kind, label, `inverted-index/quick/${kind}`));
  };
  ...
  const openFileContext = (fileExternalId: string) => {
    ...
    const tab = createInvertedIndexTab(
              ...
              <div className="idx-dashboard__quick-actions">
                ...
                <div className="idx-overview-quick-links">
                  <button
                    type="button"
                    className="idx-btn idx-btn--sm"
                    onClick={() => openQuick("inverted_index_build_metadata", "overview.quickLinks.buildMetadata")}
                  >
                  ...
                  <button
                    type="button"
                    className="idx-btn idx-btn--sm"
                    onClick={() => openQuick("inverted_index_query", "overview.quickLinks.query")}
                  >
                  ...
                  <button
                    type="button"
                    className="idx-btn idx-btn--sm"
                    onClick={() => openQuick("inverted_index_configuration", "overview.quickLinks.config")}
                  >
