...
export function SourceViewFilterNodeEditor({
  ...
  if (g === "and") {
    ...
    return (
      <div className="transform-query-filter-group" style={{ marginLeft: depth ? 12 : 0 }}>
        <div className="transform-query-toolbar" style={{ marginBottom: 6 }}>
          <span className="transform-query-hint" style={{ margin: 0, fontWeight: 600 }}>
            {t("transform.filters.filterGroupAnd")}
          </span>
          {onRemove ? (
            <button type="button" className="disc-btn disc-btn disc-btn--ghost disc-btn" onClick={onRemove}>
              {t("transform.filters.filterRemoveNode")}
            </button>
          ) : null}
        </div>
        {arr.map((child, i) => {
        ...
        >
          {t("transform.filters.filterAddToGroup")}
        </button>
      </div>
    );
  }
  ...
  if (g === "or") {
    ...
    return (
      <div className="transform-query-filter-group" style={{ marginLeft: depth ? 12 : 0 }}>
        <div className="transform-query-toolbar" style={{ marginBottom: 6 }}>
          <span className="transform-query-hint" style={{ margin: 0, fontWeight: 600 }}>
            {t("transform.filters.filterGroupOr")}
          </span>
          {onRemove ? (
            <button type="button" className="disc-btn disc-btn disc-btn--ghost disc-btn" onClick={onRemove}>
              {t("transform.filters.filterRemoveNode")}
  ...
  if (g === "not") {
    ...
    return (
      <div className="transform-query-filter-group" style={{ marginLeft: depth ? 12 : 0 }}>
        <div className="transform-query-toolbar" style={{ marginBottom: 6 }}>
          <span className="transform-query-hint" style={{ margin: 0, fontWeight: 600 }}>
            {t("transform.filters.filterGroupNot")}
          </span>
          {onRemove ? (
            <button type="button" className="disc-btn disc-btn disc-btn--ghost disc-btn" onClick={onRemove}>
              {t("transform.filters.filterRemoveNode")}
  ...
  return (
    <div className="transform-query-filter-row transform-query-filter-row--source-leaf">
      <label className="transform-query-label">
        {t("transform.filters.filterOperator")}
        <select
          className="gov-input gov-input"
          value={op}
          onChange={(e) => {
      ...
      <label className="transform-query-label">
        {t("transform.filters.filterTargetProperty")}
        <DeferredCommitInput
          className="gov-input"
      ...
      <label className="transform-query-label">
        {t("transform.filters.filterPropertyScope")}
        <DeferredCommitInput
          className="gov-input"
      ...
      <label className="transform-query-label transform-query-label--block" style={{ gridColumn: "1 / -1", display: "flex", alignItems: "center", gap: 8 }}>
        <input
          type="checkbox"
          checked={Boolean(row.negate)}
          onChange={(e) => onChange({ ...row, negate: e.target.checked })}
        />
        {t("transform.filters.filterNegate")}
      </label>
      <div key={`filter-tpl-${op}`} style={{ display: "contents" }}>
        {isRange ? (
          <div className="transform-query-filter-row--range-bounds" style={{ gridColumn: "1 / -1" }}>
            <label className="transform-query-label">
              {t("transform.filters.filterRangeGt")}
              <DeferredCommitInput
                className="gov-input"
                type="number"
            ...
            <label className="transform-query-label">
              {t("transform.filters.filterRangeGte")}
              <DeferredCommitInput
                className="gov-input"
                type="number"
            ...
            <label className="transform-query-label">
              {t("transform.filters.filterRangeLt")}
              <DeferredCommitInput
                className="gov-input"
                type="number"
            ...
            <label className="transform-query-label">
              {t("transform.filters.filterRangeLte")}
              <DeferredCommitInput
                className="gov-input"
                type="number"
          ...
          <label className="transform-query-label transform-query-label--block" style={{ gridColumn: "1 / -1" }}>
            {isComparison ? t("transform.filters.filterComparisonValue") : t("transform.filters.filterValues")}
            <DeferredCommitInput
          ...
          <p className="transform-query-hint" style={{ gridColumn: "1 / -1", margin: 0 }}>
            {t("transform.filters.filterExistsNoValues")}
          </p>
        )}
      </div>
      {onRemove ? (
        <button
          type="button"
          className="disc-btn disc-btn disc-btn--ghost disc-btn"
