...
/** Primary edges that may be spliced when a node is removed. */
export function isBridgeableFlowEdge(e: Edge, getNode: GetNode): boolean {
  const kind = edgeKind(e);
  if (kind !== "data" && kind !== "sequence" && kind !== "parallel_group") return false;
  const srcType = getNode(e.source)?.type;
  const tgtType = getNode(e.target)?.type;
  if (srcType === "etlSubgraph" || tgtType === "etlSubgraph") return false;
  return true;
}
...
export function bridgeEdgesOnNodeRemoval(edges: Edge[], toRemove: Set<string>, getNode: GetNode): Edge[] {
  ...
  for (const inE of incoming) {
    ...
    for (const outE of outgoing) {
      ...
      bridges.push({
        source: inE.source,
        sourceHandle: inE.sourceHandle ?? null,
        target: outE.target,
        targetHandle: outE.targetHandle ?? null,
      });
    }
  }
  ...
  for (const conn of bridges) {
    next = appendEtlConnectionEdge(getNode, next, conn);
  }
