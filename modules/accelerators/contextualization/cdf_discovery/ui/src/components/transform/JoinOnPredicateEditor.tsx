...
/** Operators supported by ``etl_join_on_eval.eval_join_on``. */
...
export function JoinOnPredicateEditor({ t, value, onChange, onRemove, depth = 0 }: Props) {
  ...
  if (g === "and") {
    ...
    return (
      <div className="transform-join-filter-group" style={{ marginLeft: depth ? 12 : 0 }}>
        <div className="transform-join-toolbar" style={{ marginBottom: 6 }}>
          <span className="transform-node-editor-modal__hint" style={{ margin: 0, fontWeight: 600 }}>
            {t("transform.joinEditor.groupAnd")}
          </span>
          {onRemove ? (
            <button type="button" className="disc-btn disc-btn--ghost disc-btn--sm" onClick={onRemove}>
        ...
        >
          {t("transform.joinEditor.addToGroup")}
        </button>
      </div>
    );
  }
  ...
  if (g === "or") {
    ...
    return (
      <div className="transform-join-filter-group" style={{ marginLeft: depth ? 12 : 0 }}>
        <div className="transform-join-toolbar" style={{ marginBottom: 6 }}>
          <span className="transform-node-editor-modal__hint" style={{ margin: 0, fontWeight: 600 }}>
            {t("transform.joinEditor.groupOr")}
          </span>
          {onRemove ? (
            <button type="button" className="disc-btn disc-btn--ghost disc-btn--sm" onClick={onRemove}>
              {t("transform.filters.filterRemoveNode")}
  ...
  if (g === "not") {
    ...
    return (
      <div className="transform-join-filter-group" style={{ marginLeft: depth ? 12 : 0 }}>
        <div className="transform-join-toolbar" style={{ marginBottom: 6 }}>
          <span className="transform-node-editor-modal__hint" style={{ margin: 0, fontWeight: 600 }}>
            {t("transform.joinEditor.groupNot")}
          </span>
          {onRemove ? (
            <button type="button" className="disc-btn disc-btn--ghost disc-btn--sm" onClick={onRemove}>
              {t("transform.filters.filterRemoveNode")}
  ...
  return (
    <div className="transform-query-filter-row transform-query-filter-row--join-leaf">
      <label className="gov-label">
        {t("transform.joinEditor.operator")}
        <select
          className="gov-input"
          value={op}
      ...
      <label className="gov-label">
        {t("transform.joinEditor.leftProperty")}
        <input
          className="gov-input"
      ...
      <label className="gov-label">
        {t("transform.joinEditor.rightProperty")}
        <input
          className="gov-input"
