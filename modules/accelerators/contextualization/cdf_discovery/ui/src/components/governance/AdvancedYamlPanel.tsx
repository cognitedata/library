...
export function AdvancedYamlPanel({ initialContent, onSaveRaw, onAfterSave }: Props) {
  ...
  const handleSave = async () => {
    try {
      ...
      /* Stay open; caller may show an error. */
    }
  };
  ...
  const modal = open ? (
    <div
      className="gov-advanced-fs-backdrop"
      role="dialog"
      aria-modal="true"
    ...
    >
      <div className="gov-advanced-fs">
        <header className="gov-advanced-fs__header">
          ...
          <div className="gov-advanced-fs__header-actions">
            <button type="button" className="gov-btn gov-btn--ghost" onClick={requestClose}>
              {t("btn.cancel")}
            </button>
            <button type="button" className="gov-btn gov-btn--primary" onClick={() => void handleSave()}>
              {t("advanced.modalSave")}
            </button>
          </div>
        </header>
        <div className="gov-advanced-fs__main">
          <p className="gov-hint gov-hint--warn" style={{ marginTop: 0 }}>
            {t("advanced.warning1")} <code>{t("advanced.yamlCommentChar")}</code> {t("advanced.warning2")}
          </p>
          <div className="gov-advanced-search">
            <label htmlFor={searchInputId} className="gov-advanced-search__label-text">
              {t("advanced.searchLabel")}
            </label>
            <div className="gov-advanced-search__control-row">
              <input
                id={searchInputId}
                type="search"
                className="gov-input gov-advanced-search__input"
                value={searchQuery}
              ...
              />
              <div className="gov-advanced-search__actions">
                <button
                  type="button"
              ...
              <span className="gov-advanced-search__status" role="status" aria-live="polite">
                {statusText}
              </span>
            </div>
          </div>
          <p id="advanced-search-wildcard-hint" className="gov-hint" style={{ marginTop: 0, marginBottom: 0 }}>
            {t("advanced.wildcardHint")}
          </p>
          <textarea
            ref={textareaRef}
            className="gov-textarea gov-advanced-fs__editor"
            value={text}
  ...
  return (
    <div className="gov-advanced">
      <button
        type="button"
        className="gov-advanced-toggle"
