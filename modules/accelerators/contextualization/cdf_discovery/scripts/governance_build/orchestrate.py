...
from governance_build.group_naming import enrich_groups_naming_context, resolve_group_name
from governance_build.space_naming import (
    enrich_spaces_naming_context,
...
def _instance_space_ids_for_scope(
    ...
    data_type = FALLBACK_SPACE_DATA_TYPE
    ...
    for combo in combos:
        ...
        ctx = enrich_spaces_naming_context(
            ctx, data_type=data_type, combine_names=combine_with, combo=combo
        )
        out.append(
            resolve_instance_space_external_id(
                template=tmpl,
...
def run_build_groups(
    *,
    ...
    for row in rows:
        ...
        for combo in combos:
            ctx = merge_list_combo_into_context(flat, combo)
            ctx = enrich_groups_naming_context(
                ctx,
                data_type=data_type,
                levels=levels,
                combine_names=combine_with,
            ...
            group_display_name = (
                render_template_string(display_tmpl, {**ctx, "instance_space_ids": instance_ids})
                if display_tmpl
                else ""
            )
...
def run(argv: Optional[List[str]] = None) -> int:
    ...
    p.add_argument(
        "--spaces-only",
        action="store_true",
        help="Generate Space YAML only (skip groups)",
    )
    p.add_argument(
        "--groups-only",
        action="store_true",
        help="Generate Group YAML only (skip spaces)",
    )
    args = p.parse_args(argv)
    if args.spaces_only and args.groups_only:
        logger.error("Cannot use --spaces-only and --groups-only together")
        return 1
    module_root = (args.module_root or module_root_from_here()).resolve()
    ...
    if not args.dry_run:
        ...
        merged_groups = group_rels if not args.spaces_only else list(prev_state.get("groups") or [])
        save_state(module_root, merged_spaces, merged_groups)
    return 0
