...
# CDF naming: external ids use underscores; names use colons.
# https://docs.cognite.com/cdf/deploy/cdf_toolkit/references/naming_conventions
...
def merge_groups_build_config(groups: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    name_template = (
        ...
        or DEFAULT_GROUP_NAME_TEMPLATE
    )
    display_name_template = (
        _strip_str(g.get("display_name_template"))
        or _strip_str(glob.get("display_name_template"))
        or _strip_str(expansion.get("display_name_template"))
        or DEFAULT_GROUP_DISPLAY_NAME_TEMPLATE
    )
    return {
...
def merge_spaces_build_config(spaces: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    name_template = (
        ...
        or _strip_str(glob.get("name_template"))
        or _strip_str(expansion.get("name_template"))
        or DEFAULT_SPACE_NAME_TEMPLATE
    )
    return {
        "combine_with": combine_with,
        "nodes": nodes,
        "template": template,
        "output_dir": output_dir,
        "instance_space_id_template": instance_space_id_template
        or DEFAULT_INSTANCE_SPACE_ID_TEMPLATE,
        "name_template": name_template,
        "global": glob,
    }
