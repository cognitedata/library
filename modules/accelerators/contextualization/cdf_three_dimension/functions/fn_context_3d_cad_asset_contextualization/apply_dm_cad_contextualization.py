...
def _apply_contextualization(
    ...
    *,
    ...
    seen = set()
    for r in rows:
        aid = r.columns.get("assetExternalId") or r.columns.get("assetId")
        nid = r.columns.get("nodeId")
    ...
    for i in range(0, len(items), batch_size):
        ...
        # Fetch a fresh token per batch so long runs don't hit token expiry
        ...
        log.info(f"Batch {i // batch_size + 1}/{total_batches}: {len(batch)} items OK")
        time.sleep(0.1)
