...
export function AdvancedYamlPanel({ initialContent, onSaveRaw, onAfterSave }: Props) {
  ...
  const modal = open ? (
    <div
      className="fas-advanced-fs-backdrop"
      role="dialog"
      aria-modal="true"
      aria-labelledby="advanced-yaml-dialog-title"
    >
      <div className="fas-advanced-fs">
        <header className="fas-advanced-fs__header">
          <h2 id="advanced-yaml-dialog-title" className="fas-advanced-fs__title">
            {t("advanced.modalTitle")}
          </h2>
          <div className="fas-advanced-fs__header-actions">
            <button type="button" className="fas-btn fas-btn--ghost" onClick={requestClose}>
              {t("btn.cancel")}
            </button>
            <button type="button" className="fas-btn fas-btn--primary" onClick={() => void handleSave()}>
              {t("advanced.modalSave")}
            </button>
          </div>
        </header>
        <div className="fas-advanced-fs__main">
          <p className="fas-hint fas-hint--warn" style={{ marginTop: 0 }}>
            {t("advanced.warning1")} <code>#</code> {t("advanced.warning2")}
          </p>
          <div className="fas-advanced-search">
            <label htmlFor={searchInputId} className="fas-advanced-search__label-text">
              {t("advanced.searchLabel")}
            </label>
            <div className="fas-advanced-search__control-row">
              <input
                id={searchInputId}
                type="search"
                className="fas-input fas-advanced-search__input"
                value={searchQuery}
              ...
              />
              <div className="fas-advanced-search__actions">
                <button
                  type="button"
              ...
              <span className="fas-advanced-search__status" role="status" aria-live="polite">
                {statusText}
              </span>
            </div>
          </div>
          <p id="advanced-search-wildcard-hint" className="fas-hint" style={{ marginTop: 0, marginBottom: 0 }}>
            {t("advanced.wildcardHint")}
          </p>
          <textarea
            ref={textareaRef}
            className="fas-textarea fas-advanced-fs__editor"
            value={text}
  ...
  return (
    <div className="fas-advanced">
      <button
        type="button"
        className="fas-advanced-toggle"
