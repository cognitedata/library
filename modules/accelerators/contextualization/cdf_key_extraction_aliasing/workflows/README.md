...
**Removing generated files:** **`python main.py --build --clean`** deletes matching Workflow / WorkflowVersion / WorkflowTrigger YAML under **`workflows/`** (scoped by the hierarchy **`workflow`** id; independent of **`scope_build_mode`**). It shows a summary, warns the change cannot be undone, and asks you to type **`yes`** unless you pass **`--yes`** (needed for non-interactive runs). **`--dry-run --clean`** lists paths only. **No rebuild runs after clean**—run **`--build`** again to regenerate. **`workflow_template/`** and **`workflows/README.md`** are not removed. Do not confuse this with **`main.py --clean-state`**, which clears RAW pipeline tables, not these manifests.
...
- [Quickstart — local `main.py`](../docs/guides/howto_quickstart.md)
