...
class HeuristicExtractionHandler(ExtractionMethodHandler):
    ...
    def _positional_detection(
        self, text: str, strategy: Dict, rule: ExtractionRule
    ) -> Dict[str, float]:
        ...
        for rule_config in rules_config:
            ...
            # Calibrated for parity with regex: start/end ~0.90, contains ~0.80
            if position == "start_of_field":
                match = re.match(pattern, text)
                if match:
                    candidate = match.group(0)
                    if candidate not in candidates:
                        candidates[candidate] = 0.88
                    candidates[candidate] = (
                        candidates.get(candidate, 0.88) + confidence_boost
                    )
                ...
                for keyword in keywords:
                    ...
                    for match in matches:
                        candidates[match] = 0.88 + confidence_boost
                        ...
                        for match in fallback_matches:
                            ...
                            if any(
                                ...
                                if match not in candidates:
                                    candidates[match] = 0.75
                                candidates[match] = (
                                    candidates.get(match, 0.75) + confidence_boost
                                )
                ...
                for match in matches:
                    if match not in candidates:
                        candidates[match] = 0.78
                    candidates[match] = candidates.get(match, 0.78) + confidence_boost

        return candidates

    def _frequency_analysis(
        self, text: str, strategy: Dict, rule: ExtractionRule, context: Dict[str, Any]
    ) -> Dict[str, float]:
            ...
            for substr, count in substr_freq.items():
                if count >= min_freq:
                    if substr not in candidates:
                        candidates[substr] = 0.55  # Parity tier 3 (moderate)
                    else:
    ...
    def _context_inference(
        ...
        if "surrounding_keywords" in rule_config:
            ...
            for candidate in potential_candidates:
                score = 0.55  # Parity tier 3 (moderate/context)
    ...
    def _example_based_learning(
        ...
        for rule_config in rules_config:
            if rule_config.get("learning_mode") == "similarity":
                ...
                for candidate in potential_candidates:
                    score = 0.50  # Parity tier 3 base

                    # Simple similarity scoring based on character patterns (tier 2)
                    if re.match(r"^[A-Z]+[-_]?\d+[A-Z]?$", candidate):
                        score += 0.25

                    # Boost for common industrial patterns (tier 1 parity)
                    if re.match(r"^[A-Z]{1,3}[-_]?\d{2,4}[A-Z]?$", candidate):
                        score += 0.15

                    candidates[candidate] = min(score, 1.0)
