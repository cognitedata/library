...
class ValidationConfig(BaseModel):
    min_confidence: float = Field(
        0.3,
        description="The minimum confidence a token must have to be returned in the set of results for an entity"
    )

    blacklist_keywords: List[str] = Field(
        [],
        description="Keywords that aren't allowed in the result set"
    )
...
class Config(BaseModel):
    ...
    # Remove this thing right here, fix it
    data: ConfigData
