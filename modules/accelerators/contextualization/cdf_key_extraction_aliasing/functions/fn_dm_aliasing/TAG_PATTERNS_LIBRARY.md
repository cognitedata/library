...
The **`alphanumeric_tag`** entry should stay aligned with the **`&alphanumeric_tag`** anchor in `workflow.local.config.yaml` at module root (shared equipment-tag regex for extraction). See [key extraction / aliasing report](../../docs/key_extraction_aliasing_report.md).
