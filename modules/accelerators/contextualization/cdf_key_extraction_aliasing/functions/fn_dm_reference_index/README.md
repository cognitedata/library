...
| `configuration` | **v5**: trimmed v1 scope on `workflow.input`; sets `enable_reference_index` from `key_extraction.config.parameters`, fills `config` from `aliasing` when enabled |
