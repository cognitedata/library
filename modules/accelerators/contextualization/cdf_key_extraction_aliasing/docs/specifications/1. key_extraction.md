...
**2 extraction handlers** (per rule, YAML `handler:`):
1. **`regex_handler`** - Declarative **`fields`** with optional per-field **`regex`**, trim-only when `regex` is omitted, optional **`result_template`** (all field specs are merged)
2. **`heuristic`** - Strategy-based candidates from **`parameters`**; same **`validation`** as other handlers
...
**Repository default (CDM):** `workflow.local.config.yaml` at module root defines **regex-only** rules on CogniteAsset / CogniteFile / CogniteTimeSeries with a shared **`alphanumeric_tag`** anchor (mirrored in `config/tag_patterns.yaml`). Names like `VAL_45-TT-92506:…` require the opening **`(?:\b|(?<=_))`** fragment so the match can start after `_` (Python `\b` does not separate `_` from digits). Narrative summary: [key_extraction_aliasing_report.md](../key_extraction_aliasing_report.md). Examples under `config/examples/` often add heuristics and extra rules.
...
- **Foreign Key Discovery**: Build a reference index of relationships between entities based on extracted keys
...
| `method` | String | Extraction method to use | `"regex"`, `"heuristic"` |
...
| `batch_size` | Page size for **`fn_dm_incremental_state_update`** `instances.list` pagination (`min` with 1000); not a total instance cap | `1000` (default) |
...
The rule **`handler`** selects the implementation: **`regex_handler`** or **`heuristic`**. Regex-based extraction lives **on each field** as optional **`regex`** (omit `regex` for trim-only / “passthrough” behavior).

#### regex_handler
Declarative per-field extraction: optional **`regex`** on each `fields[]` entry, optional **`result_template`**, and shared **`validation`** (including **`confidence_match_rules`**, same shape as aliasing).
...
The engine does **not** include a token-reassembly extraction method. Model composite or hierarchical identifiers with **regex** (including capture groups and `reassemble_format` where supported) or **heuristic** rules; use **aliasing** when multiple fields must be merged into one string.
      ...
      - name: "example_based_learning"
        ...
        rules:
          - learning_mode: "similarity"
            known_examples:
              source: "reference_index"  # Load from existing extracted tags
              min_examples: 50
...
Initial confidence still flows through the same **`validation`** / **`confidence_match_rules`** path as regex-derived keys.

## Confidence score calculation by handler

Each rule computes a **confidence score** (0.0–1.0) for every extracted key. Keys are only emitted if their score is ≥ the rule’s `min_confidence`, and the global `validation.min_confidence` is applied to the final result. **`confidence_match_rules`** (same idea as aliasing) run afterward.
...
1. **Per-strategy scores** (calibrated for parity with regex; exact values in code):
...
When **`regex`** is omitted, the engine assigns a **high default confidence** to the trimmed full-field value (then **`validation`** / **`confidence_match_rules`** apply as usual).
...
- **Aliasing / shared tag regex**: [Aliasing specification](2.%20aliasing.md), [`config/tag_patterns.yaml`](../../config/tag_patterns.yaml) (see [TAG_PATTERNS_LIBRARY.md](../../functions/fn_dm_aliasing/TAG_PATTERNS_LIBRARY.md))
