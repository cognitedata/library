...
5. **Passthrough** - Use the entire field value as the key (no parsing; e.g. `name` or `externalId` as-is)
...
**Repository default (CDM):** `config/scopes/default/key_extraction_aliasing.yaml` defines **regex-only** rules on CogniteAsset / CogniteFile / CogniteTimeSeries with a shared **`alphanumeric_tag`** anchor (mirrored in `config/tag_patterns.yaml`). Names like `VAL_45-TT-92506:…` require the opening **`(?:\b|(?<=_))`** fragment so the match can start after `_` (Python `\b` does not separate `_` from digits). Narrative summary: [key_extraction_aliasing_report.md](../key_extraction_aliasing_report.md). Examples under `config/examples/` often add fixed width, heuristics, and extra rules.
...
**Self-reference filter:** After validation, `KeyExtractionEngine` may remove foreign key references whose string `value` **exactly equals** a candidate key `value` on the **same** instance (`_ignore_self_referencing_keys`). Precedence: optional **`ignore_self_referencing_keys`** on each **`source_views`** entry (passed into `extract_keys` for that view’s entities), else **`parameters.ignore_self_referencing_keys`** (boolean default `true`, or a legacy dict with `default` and per-`entity_type` keys). The default CDM scope uses **`parameters: true`** and sets **`ignore_self_referencing_keys: false`** on the **CogniteTimeSeries** source view only.
...
Uses the **entire field value** as the extracted key with no pattern matching or parsing. The value is trimmed of leading/trailing whitespace and emitted as a single candidate key (or foreign key / document reference, depending on `extraction_type`). Useful when the source field already contains the identifier (e.g. `name`, `externalId`) and no extraction logic is needed.

**When to Use**:
- The field value is already the identifier (e.g. asset name = tag)
- External IDs or primary keys should be used as candidate keys as-is
- Avoiding regex or heuristics when the source is already structured
...
extraction_rules:
  - name: "name_as_candidate_key"
    ...
    description: "Use asset name field as the candidate key (no parsing)"
    enabled: true
    priority: 50
    parameters:
...
- `min_confidence` (optional): Confidence assigned to the emitted key (default `1.0`). No pattern scoring is applied.
...
Passthrough does not compute a score from content. The rule's **`min_confidence`** (or the optional `parameters.min_confidence`) is used as the key's confidence; the default is **1.0**. No pattern or positional scoring is applied.
