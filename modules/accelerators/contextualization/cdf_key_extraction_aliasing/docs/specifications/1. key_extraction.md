...
| Topic | Behavior |
|-------|----------|
| **What is indexed** | Values extracted as **foreign key** and **document** references—not **candidate_key** list columns used by **`fn_dm_aliasing`**. |
| **Aliasing rules** | The inverted-index function constructs an **inline** **`AliasingEngine`** from a copy of **`aliasing.config`** in the v1 scope so each referenced string can be expanded with the same rule/validation **shape** as the aliasing spec; it does **not** read alias rows from the aliasing RAW table. |
...
**Operational references:** [Configuration guide — Inverted index parameters](../guides/configuration_guide.md#inverted-index-parameters), [`functions/fn_dm_inverted_index/README.md`](../../functions/fn_dm_inverted_index/README.md), [Workflows README](../../workflows/README.md), [Module functional document](../module_functional_document.md#35-inverted-index).
      ...
      - name: "example_based_learning"
        ...
        rules:
          - learning_mode: "similarity"
            known_examples:
              source: "inverted_index"  # Load from existing extracted tags
              min_examples: 50
