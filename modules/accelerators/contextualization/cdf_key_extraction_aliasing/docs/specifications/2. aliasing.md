...
**Rule `type` values** the **`AliasingEngine`** registers (see [module functional document](../module_functional_document.md#321-aliasing-transformer-handlers-type--handler)):
...
10. **Pattern recognition** — ISA-style detection; enriches `context` when `tag_pattern_library` loads ([ISA patterns](../../functions/fn_dm_aliasing/ISA_PATTERNS_USAGE.md))
...
**Inverted index (optional, CDF):** When **`enable_inverted_index`** is true in scope, **`fn_dm_inverted_index`** runs after key extraction and reuses the **`aliasing.config`** shape (rules, validation) **inside that function** to expand **foreign key and document reference strings** before writing RAW inverted-index rows. It does **not** consume **`fn_dm_aliasing`** RAW output or candidate-key columns. See the [key extraction specification — Inverted index (downstream RAW)](1.%20key_extraction.md#inverted-index-downstream-raw).
...
**Runtime note:** `AliasingEngine` does **not** register a handler for `type: composite`; use multiple rules ordered by **`priority`** instead (see [module functional document](../module_functional_document.md#322-composite-and-rule-ordering)). The YAML below shows the *conceptual* shape only.
...
- **Full reference**: [Configuration guide — § Alias mapping table (RAW catalog)](../guides/configuration_guide.md#14-alias-mapping-table-raw-catalog) (`raw_table`, `scope`, `source_match`, `module.py` / `AliasingEngine` client requirement).
...
In **CDF** workflows shipped with this module, the **RAW inverted index** is maintained by **`fn_dm_inverted_index`**: a dedicated function task that reads **`FOREIGN_KEY_REFERENCES_JSON`** / **`DOCUMENT_REFERENCES_JSON`** from the key-extraction RAW table and writes lookup-token rows to the configured inverted-index RAW table. Scope tooling copies **`aliasing.config`** into that task so an **inline** **`AliasingEngine`** (same rule/validation **model** as this document) can expand each **referenced** string before indexing. **`fn_dm_aliasing`** remains the only consumer of **candidate-key** columns for alias generation.

The Python snippet in the next subsection is **pseudocode** for a generic “extract → alias → index” application. It is **not** the literal CDF call sequence: there is no `inverted_index` Python module in this repository; production behavior is defined by the functions linked from the [module README](../../README.md) and [configuration guide](../guides/configuration_guide.md#inverted-index-parameters).
...
- **Pattern library (YAML + code)**: [`config/tag_patterns.yaml`](../../config/tag_patterns.yaml), [`functions/fn_dm_aliasing/engine/tag_pattern_library.py`](../../functions/fn_dm_aliasing/engine/tag_pattern_library.py), [TAG_PATTERNS_LIBRARY.md](../../functions/fn_dm_aliasing/TAG_PATTERNS_LIBRARY.md)
