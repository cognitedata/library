...
**11 transformation types** (including catalog-driven aliasing):
...
11. **Alias mapping table** — `type: alias_mapping_table`: tag→alias rows from a Cognite **RAW** catalog (scoped matching); see [configuration guide](../guides/configuration_guide.md#13-alias-mapping-table-raw-catalog)
...
- **Full reference**: [Configuration guide — § Alias mapping table (RAW catalog)](guides/configuration_guide.md#13-alias-mapping-table-raw-catalog) (`raw_table`, `scope`, `source_match`, `main.py` / `AliasingEngine` client requirement).
