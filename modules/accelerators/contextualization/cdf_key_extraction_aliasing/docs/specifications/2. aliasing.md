...
**Repository default (CDM):** `key_extraction_aliasing.yaml` at module root ships a **small** aliasing stack (`semantic_expansion`, `strip_numeric_unit_prefix`, `leading_zero_normalization` for assets; `document_aliases` for files). It does **not** enable every transformation type listed above at once. Richer stacks live under `config/examples/aliasing/`. See [key_extraction_aliasing_report.md](../key_extraction_aliasing_report.md) and [configuration guide — Default CDM scope](../guides/configuration_guide.md#default-cdm-scope).
