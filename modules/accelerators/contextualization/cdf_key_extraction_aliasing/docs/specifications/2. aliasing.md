...
The aliasing process occurs after key extraction in parallel with reference index population:
...
graph TD
    ...
    RI[Reference Index<br/>Index & Lookup Storage]
...
**Considerations:**
- ⚠️ More aliases = larger reference index (use selective expansion)
...
aliasing:
  ...
  # Quality control (engine: confidence_match_rules on each alias string; see workflow scope YAML)
  validation:
    max_aliases_per_tag: 50
    min_confidence: 0.01
...
class ContextualizationPipeline:
    ...
    def __init__(self, config):
        ...
        self.reference_index = ReferenceIndex(config["reference_index"])
    ...
    async def process_entity(self, entity, context):
        ...
        # Step 3: Index in reference index
        for base_tag, aliases in all_aliases.items():
            await self.reference_index.index_aliases(
                entity_id=entity["id"],
