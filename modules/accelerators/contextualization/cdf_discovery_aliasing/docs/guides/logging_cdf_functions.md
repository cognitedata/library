...
Discovery pipeline entrypoints (`query_view`, `query_raw`, `query_classic`, `transform`, `validate`, `save_view`, `save_raw`, `save_classic`, `join`, `inverted_index`, `discovery_raw_cleanup` — see [`functions/README.md`](../../functions/README.md)) and shared engines under **`cdf_fn_common/`** expect a logger that supports:
...
- **`resolve_function_logger(data, logger=None, strict_level_names=False)`** — if `logger` is `None`, same as above (or strict level rules when `strict_level_names=True`); if a stdlib `Logger`, wraps with `StdlibLoggerAdapter`; if it already has `verbose` + `info`, uses as-is.
- **`cognite_function_logger(level, verbose, on_invalid_level=..., strict_level_names=False)`** — low-level; most code should use `function_logger_from_data`. Some legacy **`dependencies.create_logger_service`** adapters used **`strict_level_names=True`** so only exact `DEBUG|INFO|WARNING|ERROR` strings are accepted.
...
Local `dependencies.create_logger_service(level, verbose)` delegates to **`cognite_function_logger`** with the same **`strict_level_names`** semantics as **`function_logging`** (strict adapters vs normalized levels with invalid → `INFO` + preserved `verbose`).

## Local CLI (`module.py` / `local_runner.run`)

On **non-incremental** runs, [`local_runner/run.py`](../../local_runner/run.py) builds a single **`StdlibLoggerAdapter`** around the CLI root logger and passes it into discovery **`fn_dm_*`** handlers so `info` / `warning` / `verbose` lines share the same stdlib logging stream as the rest of `module.py`.
