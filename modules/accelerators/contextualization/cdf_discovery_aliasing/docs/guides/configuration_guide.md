...
- **Inverted index (optional)**: After extraction, **`fn_dm_inverted_index`** writes contextualization inverted-index entries (DM **`InvertedIndexEntry`** or RAW scoped postings in **`db_contextualization_idx`**) from FK/document JSON columns; enable with **`enable_inverted_index`** under **`key_extraction.config.parameters`** and configure storage on the inverted-index canvas node (see [Inverted index parameters](#inverted-index-parameters) and the [key extraction specification](../specifications/1.%20key_extraction.md#inverted-index-downstream))
...
The committed **default** scope is a slim **CDM template** (CogniteAsset, CogniteFile, CogniteTimeSeries). It differs from richer **examples** under `config/examples/` (which demonstrate extra rules, many aliasing transforms, etc.).
...
key_extraction:
  ...
  config:
    parameters:
      ...
      incremental_change_processing: true  # Required for CDF runs; cohort emitted by discovery query stages (e.g. fn_dm_view_query)
    data:
...
**RAW extraction store (`raw_table_key`):** Use table names such as **`key_extraction_state`** (default scope) or **`{site}_key_extraction_state`** (deployed workflows). Per-entity rows use `RECORD_KIND=entity` and `EXTRACTION_STATUS` (`success`, `failed`, `empty`). Run audit rows use `RECORD_KIND=run` and a timestamp row key; they are ignored when building instance skip lists. Key extraction writes **`FOREIGN_KEY_REFERENCES_JSON`** and **`DOCUMENT_REFERENCES_JSON`** (when rules produce FKs / document refs). **Per–source-field candidate lists** are stored under column names equal to the rule’s **`source_field`** string **uppercased** (e.g. `description` → `DESCRIPTION`; `metadata.code` → `METADATA.CODE`, including any dots). **`fn_dm_inverted_index`** reads only the FK/document JSON columns (not these candidate-key list columns) and writes contextualization index entries via the storage adapter configured on the inverted-index task. **`skip_entity_policy`:** `successful_only` (default) excludes instances only when their RAW row has `EXTRACTION_STATUS` `success` or `empty`; `failed` or missing `EXTRACTION_STATUS` means the instance is listed again. `none` never excludes from RAW (same listing effect as `run_all: true`). **`write_empty_extraction_rows`:** avoids re-querying instances that genuinely produce no keys when using `successful_only`.

#### Inverted index parameters

These keys live on **`key_extraction.config.parameters`** (enablement) and on the **inverted-index canvas node** `config` (storage and scope). They control whether the workflow’s **`fn_dm_inverted_index`** task runs and where index entries are stored. Task **`data`** produced by **`module.py build`** may also carry performance keys (`inverted_index_insert_batch_size`, `skip_inverted_index_ddl`, …); see [`functions/fn_dm_inverted_index/handler.py`](../../functions/fn_dm_inverted_index/handler.py) and [`functions/README.md`](../../functions/README.md).

| Parameter | Where | Type | Description |
|-----------|-------|------|-------------|
| **`enable_inverted_index`** | `key_extraction.config.parameters` | bool | When **`true`**, scope build / handlers populate **`fn_dm_inverted_index`** task data (including a deep copy of **`aliasing.config`** for FK/document string expansion using the **same rule model** as the aliasing spec). Default **`false`** if omitted. |
| **`index_storage_backend`** | inverted-index node `config` | `raw` \| `dm` | Storage adapter: RAW scoped postings (`db_contextualization_idx`) or DM **`InvertedIndexEntry`**. Default **`raw`**. |
| **`index_raw_database`** | inverted-index node `config` | string | RAW database when backend is **`raw`**. Default **`db_contextualization_idx`**. |
| **`index_schema_space`** | inverted-index node `config` | string | DM schema space when backend is **`dm`**. |
| **`scope`** | inverted-index node `config` | object | Match scope for index entries: `enabled`, `levels`, `scope_key_template`, `fallback_scope_key` (e.g. `site:{site}\|unit:{unit}`). |

**Not indexed on this path:** plain candidate-key list columns produced for downstream **transform** stages — only FK/document reference payloads feed the inverted index when configured that way.
...
**`incremental_change_processing: true`** (the default on `Parameters`) is required for incremental listing in **`fn_dm_view_query`**: the query stage paginates `instances.list`, applies watermark / hash skip when configured, and writes **cohort** rows to RAW (`RUN_ID`). The local module CLI uses the same merged **`configuration`** as deployed workflows.
...
extraction_rules:
  - name: rule_name                       # Unique rule identifier (also accepted: rule_id)
    handler: regex_handler                   # regex_handler
...
- **Transform `working` string:** With several **`fields`** and **no** `output_template`, the discovery transform stage feeds handlers a comma-separated list of **distinct** values in rule field order. Use ELT handlers such as **`heuristic_sampler`** for sample-driven matching on that string (see transform docs).
...
config:
  parameters:
    debug: true                           # Enable debug logging
    raw_db: db_discovery_aliasing               # RAW database name
...
**Alias persistence (`alias_writeback_property`):** **`fn_dm_view_save`** writes the generated alias list to **one property** on CogniteDescribable (`cdf_cdm` / `v1`). **Precedence:** (1) `aliasWritebackProperty` or `alias_writeback_property` in the `data` dict passed to the save handler (e.g. workflow task `data` for `fn_dm_view_save`); (2) `alias_writeback_property` in `config.parameters` from the first `*aliasing*.config.yaml` that defines it, when using `module.py`; (3) default `aliases`. Empty or whitespace-only values fall back to `aliases`. The chosen name is echoed in logs and in `data["alias_writeback_property"]` / per-entity summaries after a run.

**Foreign key persistence (`write_foreign_key_references`, `foreign_key_writeback_property`):** When `write_foreign_key_references` is true, **`fn_dm_view_save`** writes deduplicated foreign-key reference **strings** to the named property. The property must exist on the FK target view (default `cdf_cdm:CogniteDescribable:v1`; overridable via `foreign_key_writeback_view_*` / camelCase on the handler `data`). **Workflows** normally set the flag and property on the **`fn_dm_view_save`** task `data` and supply `source_raw_db` / `source_raw_table_key` so FK JSON from upstream RAW can be read. **`module.py`** ORs `write_foreign_key_references` from any `*aliasing*.config.yaml` that sets it to true, takes the first non-empty `foreign_key_writeback_property` from sorted pipeline files, and allows overrides via environment (`WRITE_FOREIGN_KEY_REFERENCES`, `FOREIGN_KEY_WRITEBACK_PROPERTY`) and CLI (`--write-foreign-keys`, `--foreign-key-writeback-property`). Enabling FK write without a non-empty property name fails fast in the save pipeline.
...
**Per-row entity type (`scope_filters.entity_type` / `conditions.entity_type`):** Which **extraction rule pipeline** runs is still chosen by **`extraction_rules[].aliasing_pipeline`** and context keys such as **`rule_name`**. Independently, each aliasing rule may set **`scope_filters.entity_type`** (or **`conditions.entity_type`**) to a scalar or list (e.g. **`[file]`**): the engine matches that against the **source view entity type** for the candidate key (the same value key extraction passes as **`entity_type`** — typically **`asset`**, **`timeseries`**, or **`file`** for Cognite files). Omit **`entity_type`** under filters to run the rule for all entity kinds. Use **`[file]`** on **`document_aliases`** so P&ID / drawing variants apply only to file candidate keys, not asset or time series tags.
...
**The shipped `fn_dm_transform` engine does not register `type: separator_normalization`.** Generate `-` / `_` / space / empty variants with **`character_substitution`** (see §1). External or legacy snippets that use `separator_normalization` will not run in this module—convert them to substitution rules.
...
If the pattern library is unavailable in the runtime environment, the handler leaves aliases unchanged and logs a warning. See [Aliasing specification](../specifications/2.%20aliasing.md) and [`config/tag_patterns.yaml`](../../config/tag_patterns.yaml).
...
**`fn_dm_transform` does not register a `composite` handler:** rules with `type: composite` log a missing-transformer warning and apply **no** transforms. Model multi-step behavior as **several rules** ordered by **`priority`** (and `preserve_original`) instead.
...
**Orchestration (`module.py`):** Passes `instance_space` (from the view config or, when omitted there, from each node’s `space`), `view_external_id`, `entity_type`, `entity_id`, and `entity_external_id` in the aliasing `context` so scoped rows match correctly. A **Cognite client** must be available to **`fn_dm_transform`** (and related stages) when any `alias_mapping_table` rule uses `raw_table`.
...
- **Config tree**: `modules/accelerators/contextualization/cdf_discovery_aliasing/config/` (`tag_patterns.yaml`, `examples/`, `configuration_manager.py`; v1 scope YAML lives at module root and under `examples/`)
- **ISA / tag patterns**: `modules/accelerators/contextualization/cdf_discovery_aliasing/config/tag_patterns.yaml`
- **ISA / tag patterns**: `modules/accelerators/contextualization/cdf_discovery_aliasing/config/tag_patterns.yaml` — see [Aliasing specification](../specifications/2.%20aliasing.md)
- **Tests**: `modules/accelerators/contextualization/cdf_discovery_aliasing/tests/`
