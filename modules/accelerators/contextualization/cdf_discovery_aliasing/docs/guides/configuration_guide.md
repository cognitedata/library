...
|-----------|------|-------------|
| **`enable_inverted_index`** | bool | When **`true`**, scope build / handlers populate **`fn_dm_inverted_index`** task data (including a deep copy of **`aliasing.config`** for FK/document string expansion using the **same rule model** as the aliasing spec). Default **`false`** if omitted. |
