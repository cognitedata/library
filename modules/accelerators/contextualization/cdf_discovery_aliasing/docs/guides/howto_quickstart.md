...
python modules/accelerators/contextualization/cdf_discovery_aliasing/module.py run \
  --config-path modules/accelerators/contextualization/cdf_discovery_aliasing/workflow.local.config.yaml
```
...
The pipeline writes **timestamped JSON** under the module’s **`local_run_results/`** directory (relative to `cdf_discovery_aliasing/`; shared with optional `run_fn_dm_*_local.py` outputs):
