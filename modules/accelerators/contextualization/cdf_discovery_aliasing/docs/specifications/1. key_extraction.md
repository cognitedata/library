...
**Downstream (optional):** FK and document reference strings are written to RAW JSON columns consumed by **`fn_dm_inverted_index`** to write **contextualization inverted-index entries** (DM or RAW scoped postings). Candidate keys are **not** sent through that function—see [Inverted index (downstream)](#inverted-index-downstream).
...
- [Inverted index (downstream)](#inverted-index-downstream)
...
## Inverted index (downstream)

Key extraction persists **foreign key** and **document reference** payloads on the same RAW table as candidate keys: columns **`FOREIGN_KEY_REFERENCES_JSON`** and **`DOCUMENT_REFERENCES_JSON`** (when rules with types `foreign_key_reference` / `document_reference` produce matches). **`fn_dm_inverted_index`** reads **only** those JSON columns and writes **contextualization inverted-index entries** (DM **`InvertedIndexEntry`** or RAW scoped postings in **`db_contextualization_idx`**).
...
| **Enablement** | Scope flag **`key_extraction.config.parameters.enable_inverted_index`** (boolean). Storage: **`index_storage_backend`**, **`index_raw_database`**, **`scope`** on the inverted-index canvas node (see [configuration guide](../guides/configuration_guide.md#inverted-index-parameters)). |
