...
**Repository default (CDM):** `workflow.local.config.yaml` at module root defines **regex-only** rules on CogniteAsset / CogniteFile / CogniteTimeSeries with a shared **`alphanumeric_tag`** anchor (mirrored in `config/tag_patterns.yaml`). Names like `VAL_45-TT-92506:…` require the opening **`(?:\b|(?<=_))`** fragment so the match can start after `_` (Python `\b` does not separate `_` from digits). Narrative summary: [key_extraction_aliasing_report.md](../key_extraction_aliasing_report.md). Examples under `config/examples/` often add extra rules and richer aliasing stacks.
...
**Operational references:** [Configuration guide — Inverted index parameters](../guides/configuration_guide.md#inverted-index-parameters), [`functions/fn_dm_inverted_index/handler.py`](../../functions/fn_dm_inverted_index/handler.py), [Workflows README](../../workflows/README.md), [Module functional document](../module_functional_document.md#35-inverted-index).
