...
**Rule `type` values** implemented by the discovery **`fn_dm_transform`** stack (see [module functional document](../module_functional_document.md)):
1. **Character substitution** — `"-"` → `["_", " ", ""]` and similar (this is also how you model “separator normalization”)
2. **Prefix/suffix** — Add/remove site or legacy prefixes
3. **Regex substitution** — Pattern/replacement lists
4. **Case transformation** — `upper`, `lower`, `title`, `preserve`
5. **Semantic expansion** (`semantic_expansion`) — Letter codes to full words (`P-101` → `PUMP-P-101`, …)
6. **Related instruments** — Instrument tags from equipment context
7. **Hierarchical expansion** — Context-driven path formats
8. **Document aliases** — P&ID / drawing / file-name variants
9. **Leading zero normalization** — Strip leading zeros in numeric tokens
10. **Pattern recognition** — ISA-style detection; enriches `context` when `tag_pattern_library` loads (see **`config/tag_patterns.yaml`** and this document’s pattern sections)
11. **Pattern-based expansion** — ISA-style alias expansion when the pattern library is available
12. **Alias mapping table** — RAW-backed tag→alias catalog; see [configuration guide](../guides/configuration_guide.md#14-alias-mapping-table-raw-catalog)

**Not registered at runtime:** `composite` — use several ordered rules by `priority` instead. **`separator_normalization`** appears in some legacy docs only; use **character substitution** for separator variants.
...
**Inverted index (optional, CDF):** When **`enable_inverted_index`** is true in scope, **`fn_dm_inverted_index`** runs after cohort materialization and reuses the **`aliasing.config`** shape (rules, validation) **inside that function** to expand **foreign key and document reference strings** before writing RAW inverted-index rows. It does **not** consume **`fn_dm_transform`** candidate-key columns or alias rows from the aliasing RAW table. See the [key extraction specification — Inverted index (downstream RAW)](1.%20key_extraction.md#inverted-index-downstream-raw).

**Repository default (CDM):** `workflow.local.config.yaml` at module root ships a **small** aliasing stack (`semantic_expansion`, **`regex_substitution`** rule `strip_numeric_unit_prefix`, `leading_zero_normalization` for assets; `document_aliases` for files). It does **not** enable every transformation type listed above at once. Richer stacks live under `config/examples/aliasing/`. See [key_extraction_aliasing_report.md](../key_extraction_aliasing_report.md) and [configuration guide — Default CDM scope](../guides/configuration_guide.md#default-cdm-scope).
...
- [Types of Alias Transformations](#types-of-alias-transformations)
  ...
  - [5. Separator variants (no `separator_normalization` type)](#5-separator-normalization)
  - [6. Semantic expansion](#6-semantic-expansion)
  ...
  - [11. Alias mapping table (RAW catalog)](#11-alias-mapping-table-raw-catalog)
...
- [Integration with Pipeline](#integration-with-pipeline)
  - [CDF inverted index (fn_dm_inverted_index)](#cdf-inverted-index-fn_dm_inverted_index)
...
The aliasing process occurs after key extraction in parallel with inverted index population:
...
**Implementation note:** The shipped **`fn_dm_transform`** stack does **not** implement `type: separator_normalization`. Achieve the same effect with **`character_substitution`** (multiple targets for `-`, `_`, space, empty string); see [§1](#1-character-substitution) and [Configuration guide — Separator variants](../guides/configuration_guide.md#5-separator-variants-character_substitution). The YAML in this section illustrates the *intent* for authors translating older snippets.
...
Rule type **`semantic_expansion`** (`SemanticExpansionHandler`): expand equipment **letter codes** to full words using **`type_mappings`**, **`format_templates`**, and optional **`auto_detect`**, with **`context.equipment_type`** when present. This enhances both exact string matching and semantic/fuzzy matching.

#### Why semantic expansion enhances semantic matching

Semantic expansion is particularly powerful for **semantic and fuzzy matching** scenarios where:
...
Semantic expansion creates semantic-rich aliases that improve matching in several ways:
...
Semantic expansion creates more aliases, which has trade-offs:
...
**Considerations:**
- ⚠️ More aliases = larger inverted index (use selective expansion)
...
**Runtime note:** **`fn_dm_transform`** does **not** register a handler for `type: composite`; use multiple rules ordered by **`priority`** instead (see [module functional document](../module_functional_document.md#322-composite-and-rule-ordering)). The YAML below shows the *conceptual* shape only.
...
### 11. Alias mapping table (RAW catalog)

Catalog-style aliasing: **not** a string pattern generator, but a **lookup** of a candidate tag against rows in Cognite **RAW** (key column, one or more alias columns, optional scope columns). Rule type: **`alias_mapping_table`**.

- **When to use**: You maintain a canonical mapping (for example legacy tag ↔ preferred tag, or site-specific synonyms) in RAW and want those strings added to the alias set when the key matches.
- **Full reference**: [Configuration guide — § Alias mapping table (RAW catalog)](../guides/configuration_guide.md#14-alias-mapping-table-raw-catalog) (`raw_table`, `scope`, `source_match`, Cognite client at runtime for **`fn_dm_transform`** when the rule loads RAW).
- **Persistence**: Aliases from this rule are merged with all other rules into the same `aliases` list for write-back to CogniteDescribable.
...
aliasing:
  ...
  # Quality control (engine: validation_rules on each alias string; see workflow scope YAML)
  validation:
    max_aliases_per_tag: 50
    min_confidence: 0.01
    validation_rules:
...
**Validation checks (per-alias, then cap):**
```yaml
validation:
  max_aliases_per_tag: 50
  min_confidence: 0.01
...
After scoring, aliases are deduplicated and truncated to `max_aliases_per_tag`.
...
### CDF inverted index (fn_dm_inverted_index)

In **CDF** workflows shipped with this module, the **RAW inverted index** is maintained by **`fn_dm_inverted_index`**: a dedicated function task that reads **`FOREIGN_KEY_REFERENCES_JSON`** / **`DOCUMENT_REFERENCES_JSON`** from the key-extraction RAW table and writes lookup-token rows to the configured inverted-index RAW table. Scope tooling copies **`aliasing.config`** into that task so the **same rule and validation model** as this document can expand each **referenced** string before indexing. **Candidate-key** alias generation runs in **`fn_dm_transform`** (and downstream validate/save stages), not in the inverted-index task.

The Python snippet in the next subsection is **pseudocode** for a generic “extract → alias → index” application. It is **not** the literal CDF call sequence: standalone `KeyExtractor` / `AliasingEngine` packages are **not** shipped in this repository; production behavior is defined by the **`fn_dm_*`** functions linked from [`functions/README.md`](../../functions/README.md), the [module README](../../README.md), and [configuration guide](../guides/configuration_guide.md#inverted-index-parameters).
...
from inverted_index import InvertedIndex  # illustrative third-party / app-local module, not shipped here
...
class ContextualizationPipeline:
    ...
    def __init__(self, config):
        ...
        self.inverted_index = InvertedIndex(config["inverted_index"])
    ...
    async def process_entity(self, entity, context):
        ...
        # Step 3: Index in inverted index
        for base_tag, aliases in all_aliases.items():
            await self.inverted_index.index_aliases(
                entity_id=entity["id"],
...
- **Key extraction**: [Key extraction specification](1.%20key_extraction.md)
- **Module overview**: [Module functional document](../module_functional_document.md), [module README](../../README.md)
- **Pattern library (YAML)**: [`config/tag_patterns.yaml`](../../config/tag_patterns.yaml) — loaded by discovery transform handlers when pattern-based rules are enabled.
- **Configuration examples**: [`config/examples/`](../../config/examples/README.md), [`config/examples/reference/reference_key_extraction_aliasing.yaml`](../../config/examples/reference/reference_key_extraction_aliasing.yaml)
