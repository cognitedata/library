...
**Rule `type` values** implemented by the discovery **`fn_dm_transform`** stack (see [Module specification](../MODULE_SPECIFICATION.md)):
...
**Inverted index (optional, CDF):** When **`enable_inverted_index`** is true in scope, **`fn_dm_inverted_index`** runs after cohort materialization and reuses the **`aliasing.config`** shape (rules, validation) **inside that function** to expand **foreign key and document reference strings** before writing contextualization index entries (DM or RAW). It does **not** consume **`fn_dm_transform`** candidate-key columns or alias rows from the aliasing RAW table. See the [key extraction specification — Inverted index](1.%20key_extraction.md#inverted-index-downstream).
...
**Runtime note:** **`fn_dm_transform`** does **not** register a handler for `type: composite`; use multiple rules ordered by **`priority`** instead (see [Module specification](../MODULE_SPECIFICATION.md#322-composite-and-rule-ordering)). The YAML below shows the *conceptual* shape only.
...
In **CDF** workflows shipped with this module, contextualization inverted-index entries are written by **`fn_dm_inverted_index`**: a dedicated function task that reads predecessor discovery payloads (and optionally FK/document JSON on key-extraction RAW) and persists entries via the configured storage adapter (**`index_storage_backend`**: DM **`InvertedIndexEntry`** or RAW scoped postings in **`db_contextualization_idx`**). Scope tooling copies **`aliasing.config`** into that task so the **same rule and validation model** as this document can expand each **referenced** string before indexing. **Candidate-key** alias generation runs in **`fn_dm_transform`** (and downstream validate/save stages), not in the inverted-index task.
...
- **Module overview**: [Module functional document](../MODULE_SPECIFICATION.md), [module README](../../README.md)
