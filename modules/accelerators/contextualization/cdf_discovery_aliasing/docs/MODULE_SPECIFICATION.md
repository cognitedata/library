...
**Runtime:** rules execute inside the discovery pipeline (**`fn_dm_transform`** / **`fn_dm_validate`**) after merge from the v1 scope. Semantics (handlers, `extraction_type`, RAW column naming) are in the [key extraction specification](specifications/1.%20key_extraction.md).
...
**Runtime:** transforms run in **`fn_dm_transform`** using the rule **`type`** values documented in the [aliasing specification](specifications/2.%20aliasing.md).
...
| Default scope narrative | [configuration_guide — Default CDM scope](guides/configuration_guide.md#default-cdm-scope), [`workflow.local.config.yaml`](../workflow.local.config.yaml) |
