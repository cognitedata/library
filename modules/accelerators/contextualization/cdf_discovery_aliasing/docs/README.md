# Documentation map — `cdf_discovery_aliasing`

All paths are relative to `modules/accelerators/contextualization/cdf_discovery_aliasing/` unless noted.
...
| Document                                                                     | Contents                                                                                                               |
| ---------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| [How to add custom handlers](guides/howto_custom_handlers.md)                | Extend **`fn_dm_transform`**, **`fn_dm_validate`**, **`fn_dm_view_save`** (and related save/query handlers); redeploy functions                    |
...
| Document                                                                     | Contents                                                                                                               |
| ---------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| [Workflows README](../workflows/README.md)                                   | Single workflow `key_extraction_aliasing` (**v5**), generated YAML under `workflows/`, trimmed `workflow.input.configuration`, compiled **`canvas`** over **`fn_dm_*`** stages (see [functions README](../functions/README.md)) |
...
| [Discovery functions index](../functions/README.md) | All **`fn_dm_*`** executors (query/view/raw/classic, transform, validate, join, save/view/raw/classic, inverted index, RAW cleanup) and canvas mapping |
| [fn_dm_view_query](../functions/fn_dm_view_query/handler.py) | DM list → RAW cohort |
| [fn_dm_raw_query](../functions/fn_dm_raw_query/handler.py) | RAW query stage |
| [fn_dm_classic_query](../functions/fn_dm_classic_query/handler.py) | Classic query stage |
| [fn_dm_transform](../functions/fn_dm_transform/handler.py) | Transform stage |
| [fn_dm_validate](../functions/fn_dm_validate/handler.py) | Validation stage |
| [fn_dm_join](../functions/fn_dm_join/handler.py) | Join two RAW cohort inputs |
...
- **Discovery stages:** [`functions/README.md`](../functions/README.md) — `fn_dm_view_query`, `fn_dm_raw_query`, `fn_dm_classic_query`, `fn_dm_transform`, `fn_dm_validate`, `fn_dm_join`, `fn_dm_view_save`, `fn_dm_raw_save`, `fn_dm_classic_save`, `fn_dm_inverted_index`, `fn_dm_discovery_raw_cleanup`
- **CDF handlers:** each `functions/fn_dm_*/handler.py` (see index above)
