...
This guide helps you resolve common issues with the **`cdf_discovery_aliasing`** module. The local pipeline CLI is **`module.py`** at the module root; run it from the **repository root** with `PYTHONPATH=.` (see [Quickstart — local `module.py`](../guides/howto_quickstart.md)).
...
#### Problem: Import errors for removed standalone engines (`KeyExtractionEngine`, `AliasingEngine`, …)
...
Key discovery and aliasing **execute inside CDF Functions** under `functions/fn_dm_*` (see [`functions/README.md`](../../functions/README.md)). There is no separate `KeyExtractionEngine` / `AliasingEngine` Python package in this module.

- Run tests and scripts from the **repository root** with `PYTHONPATH=.` (see [module README — Prerequisites](../../README.md#prerequisites)).
- For `scripts/validate_workflow_version_graph.py` and pytest over `functions/` imports: from the **module root** `cdf_discovery_aliasing/`, set `PYTHONPATH="functions:scripts:$(pwd)"` (see [documentation map — Package entry points](../README.md#package-entry-points-code)).
...
python modules/accelerators/contextualization/cdf_discovery_aliasing/module.py run --dry-run --limit 5
...
3. **Iterate with `module.py run`** — Use `--dry-run`, `--limit`, and inspect `local_run_results/*.json` for per-stage row counts. Adjust `extraction_rules` / `source_fields` / `validation.min_confidence` in your v1 scope YAML.
...
#### Solutions:
1. **Check Aliasing Rules** — Confirm `aliasing.config.data` (pathways / definitions) matches what your **`fn_dm_transform`** nodes reference; run **`python module.py run --dry-run --limit 20`** and read transform/validate rows in **`local_run_results/`**.
...
Load pipeline YAMLs with your editor / `module.py run --dry-run` against a small `--limit` to verify CDF connectivity and rules without writing to DM.

### 4. Test individual components

Run **`python module.py run --verbose --limit 10`** from the repository root (`PYTHONPATH=.`) and inspect stage outputs under **`local_run_results/`**. For unit-level behaviour, add tests under **`tests/unit/`** next to the **`fn_dm_*`** package you are changing.
...
python modules/accelerators/contextualization/cdf_discovery_aliasing/module.py run --verbose --limit 10
```
