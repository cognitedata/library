...
Leaf ``operator`` values (case-insensitive names, normalized to uppercase):

- ``EQUALS`` — string equality on resolved left/right values.
- ``IEQUALS`` / ``EQUALS_IGNORE_CASE`` — case-insensitive string equality.
- ``STARTS_WITH`` — ``str(left).startswith(str(right))``.
- ``ENDS_WITH`` — ``str(left).endswith(str(right))``.
- ``CONTAINS`` — ``str(right) in str(left)`` (substring search).
"""
...
from cdf_fn_common.property_path import get_property_path
