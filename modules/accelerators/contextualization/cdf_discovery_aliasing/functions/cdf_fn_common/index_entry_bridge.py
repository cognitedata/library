...
def resolve_index_raw_sample_location(
    ...
    *,
    ...
    from .discovery_query_shared import resolve_task_config
...
def collect_contextualization_index_raw_tables(
    data: Mapping[str, Any],
    *,
    task_cfg: Mapping[str, Any],
) -> list[tuple[str, str]]:
    """Return ``(raw_db, partition_table)`` pairs for contextualization index RAW backend."""
    loc = resolve_index_raw_sample_location(data, task_cfg=task_cfg)
    if loc.get("storage_backend") != "raw":
        return []
    raw_db = str(loc.get("raw_db") or "").strip()
    raw_table = str(loc.get("raw_table") or "").strip()
    if raw_db and raw_table:
