...
def materialize_scope_confidence_refs_on_task_data(data: MutableMapping[str, Any]) -> None:
    """Expand confidence-match and extraction aliasing pipeline refs on task ``configuration``.
...
def narrow_aliasing_engine_config_for_inline_rule_names(
    ...
    Used by the local Kahn runner when merging per-task ``aliasing_rule_names`` into a narrowed
    engine config dict (same shape as CDF WorkflowVersion inlining).
    """
