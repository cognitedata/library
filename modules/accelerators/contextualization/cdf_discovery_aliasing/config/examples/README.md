...
| **`reference/`** | Field-by-field reference (`config_example_complete.yaml`), wrapped `reference_key_extraction_aliasing.yaml`. |
...
1. **Local run** (cwd = module root `cdf_discovery_aliasing/`):  
   `module.py run --config-path config/examples/key_extraction/<demo>.key_extraction_aliasing.yaml`
   or `module.py run --config-path config/examples/aliasing/aliasing_default.key_extraction_aliasing.yaml`.
   From the **repository root** (`library/`), use the full prefix `modules/accelerators/contextualization/cdf_discovery_aliasing/` on `module.py` and on these paths (see the module [README.md](../../README.md) and [Quickstart](../../docs/guides/howto_quickstart.md) for `PYTHONPATH` and `.env`).
