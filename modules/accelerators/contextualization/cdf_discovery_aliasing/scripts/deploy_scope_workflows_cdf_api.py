...
from functions.cdf_fn_common.workflow_compile.codegen import escape_workflow_version_document_for_cdf
...
def _workflow_version_body_for_cdf_api(wv_raw: dict[str, Any]) -> dict[str, Any]:
    """Strip rejected ``input`` and escape literal ``$`` in task ``data`` for CDF upsert."""
...
def deploy_scope_workflows(
    ...
    *,
    ...
    # Strip legacy ``workflowDefinition.input`` if present (CDF rejects it; scoped build no longer emits it).
    wv_api = cc.workflows.versions
    wv_body = _workflow_version_body_for_cdf_api(wv_raw)
    wv_api._create_multiple(  # noqa: SLF001 — raw API body matches Toolkit-deployable YAML shape
        [wv_body],
