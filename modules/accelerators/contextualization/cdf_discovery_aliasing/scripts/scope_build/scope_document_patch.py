...
def scope_configuration_for_workflow_trigger(scope_document: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Flattens subgraphs, strips layout positions and legacy rule lists, optionally shrinks
    ``source_views`` when ``source_view`` canvas nodes reference a subset, prunes discovery stage lists
