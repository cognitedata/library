"""Load unified scope YAML for ``build_scopes`` (graph in ``canvas`` or hoisted from root ``nodes``)."""

from __future__ import annotations

from pathlib import Path
from typing import Any, Dict

import yaml

from cdf_fn_common.scope_canvas_merge import normalize_root_graph_into_canvas


def load_scope_document_dict_for_build(scope_yaml_path: Path) -> Dict[str, Any]:
    """Parse scope YAML from *scope_yaml_path* and normalize root-level graph keys into ``canvas``."""
    if not scope_yaml_path.is_file():
        raise FileNotFoundError(f"Scope document not found: {scope_yaml_path}")
    raw = yaml.safe_load(scope_yaml_path.read_text(encoding="utf-8"))
    if not isinstance(raw, dict):
        raise ValueError(f"Scope YAML root must be a mapping: {scope_yaml_path}")
    normalize_root_graph_into_canvas(raw)
    return raw
