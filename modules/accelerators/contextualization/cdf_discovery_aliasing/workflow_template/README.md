...
| File | Role |
|------|------|
| [`workflow.template.config.yaml`](workflow.template.config.yaml) | **Unified** v1 scope template: **`source_views`**, **`key_extraction`**, **`aliasing`**, optional metadata, and embedded **`canvas`** (nodes/edges). Patched into each generated trigger’s **`input.configuration`** as a **trimmed** copy (flattened subgraphs, no layout positions, rule lists pruned to rules referenced by executable canvas nodes). Aliasing transforms live under **`aliasing.config.data.pathways`** (sequential macro step mirroring the former flat **`aliasing_rules`** list); **`aliasing_rules`** is kept empty for tooling compatibility. Same **Key Discovery** / incremental fields as before — see [workflows/README.md](../workflows/README.md) and [configuration guide](../docs/guides/configuration_guide.md). |
...
| [`workflow.template.WorkflowVersion.yaml`](workflow.template.WorkflowVersion.yaml) | WorkflowVersion **`v5`** human reference (generated **`workflows/<suffix>/*.WorkflowVersion.yaml`** inlines per-task IR fields into each function task’s **`data`** at build time). |
