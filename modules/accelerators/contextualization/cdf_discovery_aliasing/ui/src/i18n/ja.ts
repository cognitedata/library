...
export const ja: Messages = {
  ...
  "run.cdfToolsHint":
    "デプロイは既定で Discovery の Cognite 関数を更新したうえで、Cognite SDK により選択スコープの Workflow / WorkflowVersion / WorkflowTrigger を CDF に upsert します（資格情報は module.py run と同じ）。CDF で実行はワークフロー実行を開始します（KEA_WORKFLOW_CLIENT_* は任意）。トリガー YAML に ``{{instance_space}}`` が残る場合は下にインスタンススペースを入力してください。それ以外の未解決 ``{{…}}`` はこの UI でも引き続き許可されます。",
  "run.cdfScopedOnly":
