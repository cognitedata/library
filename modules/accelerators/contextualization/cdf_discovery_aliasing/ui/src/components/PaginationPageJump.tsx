...
export function PaginationPageJump({ t, pageIndex, pageCount, disabled, onPageIndexChange }: Props) {
  ...
  const inputLabel = t("queries.previewPageJumpInput", { pages: String(pageCount) });

  return (
    <label
      className="discovery-pagination__jump"
      title={inputLabel}
    >
      <input
        type="number"
        className="discovery-input discovery-pagination__jump-input"
        min={1}
        max={pageCount}
        disabled={disabled}
        value={draft}
      ...
      />
      <span className="discovery-pagination__jump-of" aria-hidden>
        {t("queries.previewPageOf", { pages: String(pageCount) })}
      </span>
    </label>
  );
}
