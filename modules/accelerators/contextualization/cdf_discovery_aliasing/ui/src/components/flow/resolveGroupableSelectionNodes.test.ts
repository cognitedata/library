...
describe("resolveGroupableSelectionNodes", () => {
  it("includes box-selected ids from rf snapshot even when node.selected is false", () => {
    const a: Node = { id: "a", type: "keaTransform", position: { x: 0, y: 0 }, data: {}, selected: false };
    const b: Node = { id: "b", type: "keaTransform", position: { x: 1, y: 1 }, data: {}, selected: false };
    const c: Node = { id: "c", type: "keaSourceView", position: { x: 2, y: 2 }, data: {}, selected: false };
    const nodes = [a, b, c];
    const rfSnap = [
      { id: "a", type: "keaTransform", position: { x: 0, y: 0 }, data: {} },
      { id: "b", type: "keaTransform", position: { x: 1, y: 1 }, data: {} },
    ] as Node[];
  ...
  it("still honors node.selected when rf snapshot is empty", () => {
    const a: Node = { id: "a", type: "keaTransform", position: { x: 0, y: 0 }, data: {}, selected: true };
    const b: Node = { id: "b", type: "keaTransform", position: { x: 1, y: 1 }, data: {}, selected: true };
    const nodes = [a, b];
