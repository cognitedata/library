...
function discoveryKindUsesNodeConfig(kind: CanvasNodeKind): boolean {
  switch (kind) {
    case "query_view":
    case "query_raw":
    case "query_classic":
...
function discoveryStageInlineNonempty(kind: CanvasNodeKind, value: unknown): boolean {
  ...
  if (
    ...
    const rawDb = row.raw_db != null ? String(row.raw_db).trim() : "";
    const rawTk =
      row.raw_table_key != null
        ? String(row.raw_table_key).trim()
        : row.raw_table != null
          ? String(row.raw_table).trim()
          : "";
  ...
  if (kind === "join") {
    ...
    return dsc.length > 0 && jo !== null && typeof jo === "object" && !Array.isArray(jo) && Object.keys(jo).length > 0;
  }
  return false;
}

function discoveryIndexDrift(kind: CanvasNodeKind, data: Record<string, unknown>): { ok: boolean; hint: string } {
  if (!discoveryKindUsesNodeConfig(kind)) return { ok: true, hint: "" };
  if (discoveryStageInlineNonempty(kind, data.config)) return { ok: true, hint: "" };
  return { ok: false, hint: "flow.inspectorDiscoveryInlineMissing" };
}
