...
# Tooling-only modules (not deployed to CDF).
EXCLUDED_MODULE_IDS = frozenset({"cdf_foundation_cicd"})
...
def discover_foundation_module_paths(modules_root: Path, repo_root: Path | None = None) -> list[str]:
    """Resolve deployable dp:foundation module paths (excludes this CI generator module)."""
    ...
    if packages_toml.is_file():
        ...
        return [p for p in listed if Path(p).name not in EXCLUDED_MODULE_IDS]
    ...
    for module_toml in sorted(modules_root.rglob("module.toml")):
        ...
        for line in text.splitlines():
            if line.strip().startswith("id = "):
                ...
                break
        if module_id in EXCLUDED_MODULE_IDS:
            continue
...
def main() -> None:
    ...
    if not modules_root.is_dir():
        print(f"modules/ not found under {repo_root}", file=sys.stderr)
        sys.exit(1)
    ...
    for env_name in ENVIRONMENTS:
        ...
        header = (
            "# Generated by cdf_foundation_cicd/scripts/generate_env_configs.py\n"
