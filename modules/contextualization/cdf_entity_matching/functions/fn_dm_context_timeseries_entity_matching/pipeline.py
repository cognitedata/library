...
try:
    ...
    def monitor_memory_usage(logger, operation_name=""):
        pass
...
def asset_entity_matching(
    ...
    try:
        ...
        logger.info(
            f"Starting entity matching function: {FUNCTION_ID} with loglevel = {data.get('logLevel', 'INFO')},  reading parameters from extraction pipeline config: {pipeline_ext_id}"
        )
        ...
        if config.parameters.run_all:
            ...
            delete_table(
                client, config.parameters.raw_db, config.parameters.raw_tale_ctx_good
            )
            delete_table(
        ...
        manual_result = apply_manual_mappings(
            client, logger, config, raw_uploader, manual_mappings, good_matches
        )
        ...
        logger.debug(
            "Get entities (ex: time series) that has been updated since last run"
        )
        new_entities = get_new_entities(
            client, config, logger, None, rule_mappings, None
        )

        logger.debug(f"Number of new entities to process are: {len(new_entities)}")
        if len(new_entities) == 0:
            logger.debug(
                "No new entities to process, we are done - just update pipeline run"
            )
            update_pipeline_run(
                client,
                logger,
        ...
        logger.debug(
            "Get all assets that are input for matching assets ( based on TAG filtering given in config)"
        )
        ...
        while len(new_entities) > 0:
            logger.info(
                "Applying rule based mappings - using provided reg expressions to match entities to assets"
            )
            good_matches = apply_rule_mappings(
                client, config, logger, good_matches, asset_dest, new_entities
            )  # type: ignore

            match_results = get_matches(
                client,
                config,
                logger,
                matching_model_id or "",
                asset_dest,
                new_entities,
            )  # type: ignore

            good_matches, bad_matches = select_and_apply_matches(
                client, config, logger, good_matches, match_results
            )  # type: ignore
            write_mapping_to_raw(
                client, config, raw_uploader, good_matches, bad_matches, logger
            )
            ...
            new_entities = get_new_entities(
                client, config, logger, list_good_matches, rule_mappings, bad_matches
            )  # type: ignore
...
def manual_table_exists(client: CogniteClient, config: Config) -> bool:
...
def rule_table_exists(client: CogniteClient, config: Config) -> bool:
    tables = client.raw.tables.list(config.parameters.raw_db, limit=None)
    return any(tbl.name == config.parameters.raw_tale_ctx_rule for tbl in tables)


def read_manual_mappings(
    client: CogniteClient, logger: CogniteFunctionLogger, config: Config
) -> list[Row]:
    manual_mappings = []
    seen_mappings = set()
    try:
        ...
        row_list = client.raw.rows.list(
            config.parameters.raw_db, config.parameters.raw_tale_ctx_manual, limit=-1
        )
        for row in row_list:
            if not (
                ...
                manual_mappings.append(
                    {
                        ...
                        COL_KEY_MAN_MAPPING_ASSET: row.columns[
                            COL_KEY_MAN_MAPPING_ASSET
                        ].strip(),
                    }
                )
...
def apply_manual_mappings(
    ...
    try:
        entity_list = [
            mapping[COL_KEY_MAN_MAPPING_ENTITY] for mapping in manual_mappings
        ]
        lookup_mapping = {
            mapping["key"]: mapping[COL_KEY_MAN_MAPPING_ASSET]
            for mapping in manual_mappings
        }

        is_filter_param = dm.filters.In(
            config.data.entity_view.as_property_ref("name"), entity_list
        )
        ...
        for entity in all_entities:
            ...
            mapping = {}
            mapping[COL_KEY_MAN_MAPPING_ENTITY] = entity.properties[entity_view_id][
                "name"
            ]
            ...
            raw_uploader.add_to_upload_queue(
                ...
                config.parameters.raw_tale_ctx_good,
                Row(entity.external_id, mapping),
            )

        logger.debug(
            f"Number of items to update based on manual updates: {len(item_update)}"
        )
        ...
        logger.error(
            f"ERROR: Not able run manual mapping for {manual_mappings} - error: {e}"
        )
        return good_matches
...
def read_rule_mappings(
    ...
    try:
        ...
        row_list = client.raw.rows.list(
            config.parameters.raw_db, config.parameters.raw_tale_ctx_rule, limit=-1
        )
        idx = 1
        for row in row_list:
            if not row.columns:
            ...
            rule_mappings.append(
                {
                    "key": f"{idx}",
                    COL_KEY_RULE_REGEXP_ENTITY: row.columns[
                        COL_KEY_RULE_REGEXP_ENTITY
                    ].strip(),
                    COL_KEY_RULE_REGEXP_ASSET: row.columns[
                        COL_KEY_RULE_REGEXP_ASSET
                    ].strip(),
                }
            )
...
def get_all_assets(
    ...
    is_selected = get_query_filter(
        type, job_config.asset_view, config.parameters.run_all, logger
    )
...
def get_new_entities(
    ...
    logger.debug(
        f"Get new entities from view: {entity_view_id}, based on config: {entity_view_config}"
    )
    is_selected = get_query_filter(
        type, entity_view_config, config.parameters.run_all, logger
    )
    ...
    bad_entity_ids = (
        {match["entity_ext_id"] for match in bad_matches} if bad_matches else set()
    )
...
def clean_asset_links(
    config: Config, entity_ext_id: str, item_update: list[NodeApply]
) -> list[NodeApply]:
    entity_view_id = config.data.entity_view.as_view_id()
...
def get_matches(
    ...
    try:
        if matching_model_id:
            ...
            update_state_store(
                client, config, logger, str(model.id), STAT_STORE_MATCH_MODEL_ID
            )
        ...
        logger.error(
            f"ERROR: Failed to get matching model and run prediction. Error: {type(e)}({e})"
        )
        raise
...
def apply_rule_mappings(
    ...
    good_matches_set = {
        f"{match['asset_ext_id']}_{match['entity_ext_id']}" for match in good_matches
    }
    ...
    try:
            ...
            for r_key_from_d2 in set2:
                if r_key_from_d2 in index1:
                    for d1_match in index1[r_key_from_d2]:
                        ...
                        pair = tuple(
                            sorted((d2["entity_ext_id"], d1_match["asset_ext_id"]))
                        )

                        if pair not in unique_matches_tracker:
                            # matches.append((d1_match, d2))
                            match_key = (
                                f"{d1_match['asset_ext_id']}_{d2['entity_ext_id']}"
                            )
                            if match_key in good_matches_set:
                                logger.debug(
                                    f"Match already exists in good matches: {d1_match['asset_ext_id']} - {d2['entity_ext_id']}"
                                )
                                continue
                            ...
                            good_matches.append(
                                {
                                    ...
                                    "asset_view_id": str(
                                        config.data.asset_view.as_view_id()
                                    ),
                                }
                            )

                            matches[d2["entity_ext_id"]][0].append(
                                d1_match["asset_ext_id"]
                            )
                            existing_asset_list = json.loads(d2["assets"])
                            if len(existing_asset_list) > 0:
                                matches[d2["entity_ext_id"]][1] = (
                                    matches[d2["entity_ext_id"]][1]
                                    + existing_asset_list
                                )
        ...
        for entity_ext_id in matches:
            cnt += 1
            asset_ext_ids = matches[entity_ext_id][
                0
            ]  # Assuming the first asset is the one to match with
        ...
        logger.info(
            f"Number of items to update based on rule mapping: {len(item_update)}"
        )
        if not config.parameters.debug:
            # Apply the updates to the data model
            client.data_modeling.instances.apply(item_update)
...
def select_and_apply_matches(
    ...
    try:
        for match in match_results:
            if match["matches"]:
                if (
                    match["matches"][0]["score"]
                    >= config.parameters.auto_approval_threshold
                ):
                    ...
                    if match_key in good_matches_set:
                        logger.debug(
                            f"Match already exists in good matches: {asset_ext_id} - {entity_ext_id}"
                        )
                        continue
                    else:
                        good_matches_set.add(match_key)

                    new_good_matches.append(
                        add_to_dict(match, str(entity_view_id), str(asset_view_id))
                    )
                else:
                    bad_matches.append(
                        add_to_dict(match, str(entity_view_id), str(asset_view_id))
        ...
        logger.info(
            f"INFO: Got {len(new_good_matches)} matches with score >= {config.parameters.auto_approval_threshold}"
        )
        logger.info(
            f"INFO: Got {len(bad_matches)} matches with score < {config.parameters.auto_approval_threshold}"
        )
        ...
        print(
            f"ERROR: Failed to parse results from entity matching - error: {type(e)}({e})"
        )
...
def add_to_items(
    ...
    if entity_assets:
        ...
        if len(entity_assets_array) > 0:  # Check if there are existing assets
            for asset in entity_assets_array:
                assets.append(
                    DirectRelationReference(
                        space=asset["space"], external_id=asset["externalId"]
                    )
                )
    ...
    for asset_ext_id in asset_ext_ids:
        if not asset_ext_id:
            logger.warning(
                f"Asset external ID is empty for entity: {entity_ext_id}, skipping"
            )
            continue
        if asset_ext_id in [asset.external_id for asset in assets]:
            logger.debug(
                f"Asset: {asset_ext_id} already exists in entity: {entity_ext_id}, skipping"
            )
            continue
        logger.debug(f"Adding asset: {asset_ext_id} to entity: {entity_ext_id}")
        assets.append(
            DirectRelationReference(
                space=config.data.asset_view.instance_space, external_id=asset_ext_id
            )
        )
...
def write_mapping_to_raw(
    ...
    try:
        logger.info(
            f"INFO: Clean up BAD table: {raw_db}/{raw_tale_ctx_bad} before writing new status"
        )
        ...
        for match in good_matches:
            raw_uploader.add_to_upload_queue(
                raw_db, raw_tale_ctx_good, Row(match["entity_ext_id"], match)
            )  # type: ignore
        logger.info(f"INFO: Added {len(good_matches)} to {raw_db}/{raw_tale_ctx_good}")

        for not_match in bad_matches:
            raw_uploader.add_to_upload_queue(
                raw_db, raw_tale_ctx_bad, Row(not_match["entity_ext_id"], not_match)
            )  # type: ignore
        ...
        logger.error(
            f"ERROR: Failed to write mapping to RAW DB - error: {type(e)}({e})"
        )
