...
from config import Config, ViewPropertyConfig, QueryConfig
...
def asset_entity_matching(
    ...
    try:
        ...
        # TODO this is where we grab the target entities
        target_entities = get_target_entities(client, config, logger, None, rule_mappings, None)

        logger.debug(f"Number of new entities to process are: {len(target_entities)}")
        ...
        # TODO this is where we grab the source entities
        source_entities = get_all_assets(client, logger, config, rule_mappings)
        if len(source_entities) == 0:
            logger.warning(f"No assets found based on configuration, please check the configuration")
            update_pipeline_run(client, logger, pipeline_ext_id, "success", match_count, not_matches_count, None)
            return

        while len(target_entities) > 0:

            logger.info("Applying rule based mappings - using provided reg expressions to match entities to assets")
            good_matches = apply_rule_mappings(client, config, logger, good_matches, source_entities, target_entities)  # type: ignore

            # TODO refactor function so that match_from = source_entities and match_to = target_entities
            match_results = get_matches(client, config, logger, matching_model_id or "", source_entities, target_entities)  # type: ignore
            ...
            new_entities = get_target_entities(client, config, logger, list_good_matches, rule_mappings, bad_matches)  # type: ignore
...
def read_manual_mappings(
    ...
    try:
        ...
        for row in row_list:
            ...
            if not row.columns:
                continue
            entity = row.columns[COL_KEY_MAN_MAPPING_TARGET].strip()
            if entity not in seen_mappings:
                seen_mappings.add(entity)
                manual_mappings.append(
                    {
                        "key": row.key,
                        COL_KEY_MAN_MAPPING_TARGET: entity,
                        COL_KEY_MAN_MAPPING_SOURCE: row.columns[COL_KEY_MAN_MAPPING_SOURCE].strip(),
                    }
                )
...
def apply_manual_mappings(
    ...
    try:
        entity_list = [mapping[COL_KEY_MAN_MAPPING_TARGET] for mapping in manual_mappings]
        lookup_mapping = {mapping["key"]: mapping[COL_KEY_MAN_MAPPING_SOURCE] for mapping in manual_mappings}
        ...
        for entity in all_entities:
            ...
            mapping = {}
            mapping[COL_KEY_MAN_MAPPING_TARGET] = entity.properties[entity_view_id]["name"]
            mapping[COL_KEY_MAN_MAPPING_SOURCE] = asset_ext_id
            ...
            rule_mappings.append(
                {
                        ...
                        COL_KEY_RULE_REGEXP_SOURCE: row.columns[COL_KEY_RULE_REGEXP_SOURCE].strip(),
                }
            )
            idx += 1
...
# TODO Change this to handle the retrieval of both source and target entities
def get_all_assets(
    client: CogniteClient,
    logger: CogniteFunctionLogger,
    config: Config,
    ...
    search_property = job_config.source_query.target_view.search_property

    is_selected = get_query_filter(type, job_config.source_query, config.parameters.run_all, logger)

    all_assets = client.data_modeling.instances.list(
        space=job_config.asset_view.instance_space,
        sources=[job_config.source_query.target_view.as_view_id()],
        filter=is_selected,
        limit=-1
    )
    ...
    for asset in all_assets:
        ...
        org_name = str(asset.properties[job_config.source_query.target_view.as_view_id()]["name"])

        rule_keys = []
        if rule_mappings:
            for rule in rule_mappings:
                reg_exp = str(rule[COL_KEY_RULE_REGEXP_SOURCE])
        ...
        if search_property in asset.properties[job_config.source_query.target_view.as_view_id()]:
            match_properties = asset.properties[job_config.source_query.target_view.as_view_id()][search_property]
...
def get_query_filter(
    type: str,
    entity_config: QueryConfig,
    run_all: bool,
    logger: CogniteFunctionLogger,
) -> dm.filters.Filter:
    view_config = entity_config.target_view
...
def apply_rule_mappings(
    ...
    try:
        # Build an inverted index for source_entities
        # Format: { 'rule_key_value': [dict_from_source_entities_1, dict_from_source_entities_2, ...] }
        index1 = defaultdict(list)
        matches = defaultdict(lambda: [[], []])

        for d1 in source_entities:
...
def select_and_apply_matches(
    ...
    try:
        ...
        # Removing this for now as we don't want to write new matches just yet
