    ...
    try:
        ...
        client.extraction_pipelines.runs.create(
            ExtractionPipelineRun(
                extpipe_external_id=xid, status=status, message=truncated_msg
            )
        )
...
def get_new_items(
    ...
    try:
        ...
        if node_type == TS_NODE:
            ...
            debug_item = (
                config.parameters.debug_timeseries if config.parameters.debug else None
            )
            filter_query = get_ts_filter(
                view_config, debug_item, config.parameters.run_all, logger
            )
        else:  # ASSET_NODE
            view_config = config.data.job.asset_view
            filter_query = get_asset_filter(
                view_config, logger, config.parameters.run_all
            )
