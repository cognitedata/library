...
class TestStaging(unittest.TestCase):
    ...
    def test_clear_finished_matches_deletes_raw_rows_in_batches(self) -> None:
        """RAW delete accepts at most 1000 keys per request."""
        db, table = "db", "good"
        for index in range(1001):
            self.client.raw.rows.insert(db, table, Row(f"row-{index}", {}))
        delete_batch_sizes: list[int] = []
        original_delete = self.client.raw.rows.delete

        def tracking_delete(db_name: str, table_name: str, keys: str | Sequence[str]) -> None:
            delete_batch_sizes.append(len(keys) if isinstance(keys, list) else 1)
            original_delete(db_name, table_name, keys)

        self.client.raw.rows.delete = tracking_delete  # type: ignore[method-assign]

        deleted = clear_finished_matches(self.client, self.config, self.logger, table)  # type: ignore[arg-type]

        self.assertEqual(deleted, 1001)
        self.assertEqual(delete_batch_sizes, [1000, 1])
