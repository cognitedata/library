"""Entity Matching Submit

Runs the manual and rule based matching, stages the result, and starts the CDF predict
job. The results of that job are picked up by fn_dm_context_entity_matching_collect.
"""
...
def handle(data: dict[str, Any], client: CogniteClient) -> dict[str, Any]:
    """Start an entity matching predict job.
    ...
    try:
        ...
        logger.info(f"Starting Entity Matching Submit with loglevel = {loglevel}")
        ...
        benchmark.benchmark_function("Submit pipeline", submit_entity_matching, client, logger, data, config)

        cleanup_memory()
        monitor_memory_usage(logger, "Handler end")
        benchmark.log_summary()

        logger.info("Entity matching submit completed successfully!")
        return {"status": "succeeded", "data": data}

    except Exception as e:
        message = f"Entity matching submit failed: {e!s}"
...
def run_locally() -> dict[str, Any]:
    """Run the submit function against a CDF project from environment variables."""
    ...
    client = CogniteClient(
        ClientConfig(
            client_name="Entity Matching Submit",
            base_url=base_url,
