...
| `cicd_clientId`, `cicd_clientSecret` | Optional: schedules / automation. **Not module variables** — supply at deploy time via env/secret store (see `env.template`); functions read them via `client.config` / `os.environ`. |
