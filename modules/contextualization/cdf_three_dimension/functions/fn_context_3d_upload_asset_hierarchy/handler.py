...
def _apply_nodes_with_retry(client: CogniteClient, batch: list[NodeApply]) -> int:
    for attempt in range(_APPLY_MAX_RETRIES):
        try:
            result = client.data_modeling.instances.apply(nodes=batch)
            upserted = len(result.nodes or [])
            if upserted != len(batch):
                raise RuntimeError(
                    f"instances.apply returned {upserted} nodes for batch size {len(batch)}"
                )
            return upserted
        except CogniteAPIError as e:
            code = getattr(e, "code", None)
            if code in _RETRYABLE_STATUS_CODES and attempt < _APPLY_MAX_RETRIES - 1:
...
def handle(data: dict, client: CogniteClient) -> dict:
    ...
    """
    pipeline_ext_id = data.get("ExtractionPipelineExtId") or os.environ.get("EXTRACTION_PIPELINE_EXT_ID")
    if not pipeline_ext_id:
        raise ValueError(
            "Missing ExtractionPipelineExtId in run input and EXTRACTION_PIPELINE_EXT_ID env var."
        )
    ...
    if not file_list:
        raise FileNotFoundError(f"File not found: externalId={file_ext_id!r}")
    content = client.files.download_bytes(file_list[0].id)

    try:
        # Accept UTF-8 with/without BOM; fail explicitly for other encodings.
        text = content.decode("utf-8-sig")
    except UnicodeDecodeError as e:
        raise ValueError(
            f"Could not decode CSV as UTF-8: {e}. Please upload the file encoded as UTF-8."
        ) from e
    ...
    for i in range(0, total, BATCH_SIZE):
        ...
        upserted += _apply_nodes_with_retry(client, batch)

    return {"status": "succeeded", "nodesUpserted": upserted, "assetInstanceSpace": space}
