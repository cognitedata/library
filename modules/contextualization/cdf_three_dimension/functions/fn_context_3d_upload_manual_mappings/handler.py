...
def handle(data: dict, client: CogniteClient) -> dict:
    ...
    raw_db = config.get("rawdb") or data.get("rawdb")
    if not raw_db:
        raise ValueError("Missing 'rawdb' in pipeline config or run input.")
    ...
    for i in range(0, len(rows), _RAW_INSERT_BATCH_SIZE):
        client.raw.rows.insert(
            db_name=raw_db,
            table_name=raw_table,
            row=rows[i : i + _RAW_INSERT_BATCH_SIZE],
        )
