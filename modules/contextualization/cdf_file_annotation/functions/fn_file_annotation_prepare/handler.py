...
def _create_local_prepare_service(config, client, logger, tracker, function_call_info) -> AbstractPrepareService:
    data_model_instance: IDataModelService = create_general_data_model_service(config, client, logger)
    prepare_instance: AbstractPrepareService = LocalPrepareService(
        client=client,
        config=config,
        logger=logger,
        tracker=tracker,
