...
from pipeline import file_metadata_update
...
def handle(data: dict, client: CogniteClient) -> dict:
    try:
        ...
        logger.info(f"Starting update of metadata with loglevel = {loglevel},  reading parameters from extraction pipeline config: {data.get('ExtractionPipelineExtId')}")
        config = load_config_parameters(client, data)
        logger.debug("Loaded config successfully")
        file_metadata_update(client, logger, data, config)
...
def run_locally():
    ...
    data = {"logLevel":"INFO", "ExtractionPipelineExtId": "ep_ctx_metadata_update"}
    handle(data, client)
