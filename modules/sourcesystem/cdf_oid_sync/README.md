...
```bash
cdf modules add -d cdf_oid_sync
```
...
2. Use **Space** to select **cdf_oid_sync**.
3. Press **Enter**.
...
Edit `modules/sourcesystem/cdf_oid_sync/default.config.yaml`:
...
environment:
  ...
  validation-type: dev
  selected:
    - modules/sourcesystem/cdf_oid_sync  # Add this line
