...
select
  ...
  cast(coalesce(`WORKORDER_TASKNAME`, `WORKORDER_ITEMINFO`) as STRING) as name,
  cast(`WORKORDER_STATUS` as STRING) as status,
  array(cast(`WORKORDER_ITEMNAME` as STRING)) as sysTagsFound,
  'SAP Operations' as sourceContext,
  cast(
    concat_ws(
      ' - ',
      `WORKORDER_NUMBER`,
      `WORKORDER_ITEMTAGHIGHCRITICAL`,
      `WORKORDER_ITEMINFO`
    ) as STRING
  ) as description
from
