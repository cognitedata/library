...
variables:
  modules:
    cdf_files_foundation:
      location: "site1"                                       # Site code, used in externalIds (ep_<location>_files_sharepoint)
      dataset: "ds_files"                                     # dataSetExternalId for the pipeline (Files land in this data set)
      instanceSpace: "sp_instances"                           # DM space where CogniteFile instances are written
...
6. **Confirm `data_model.space`** matches your `instanceSpace` — `CogniteFile`
   ...
   already exists (deploy `models/cdf_process_industry_extension` or your DM
   space module first).
...
- DM space configured for `CogniteFile` instances (e.g. via `models/` modules) — required because the example uses `destination-mode: cdm`
...
```bash
cdf deploy modules/sourcesystem/cdf_files_foundation --env your-environment
```
